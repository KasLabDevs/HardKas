...
export function ActivityTimelineCompact() {
  ...
  const getIcon = (type: string) => {
    ...
    if (type.includes('signed')) return <ShieldCheck size={14} className="text-cyan-400" />;
    if (type.includes('receipt')) return <ShieldCheck size={14} className="text-emerald-400" />;
    ...
    <div className="space-y-2">
      ...
      <div className="space-y-2">
        {events.length === 0 ? (
          ...
          events.map((ev, i) => (
            <div key={`${ev.artifactId}-${i}`} className="bg-zinc-950 p-2 rounded border border-zinc-800 flex items-center justify-between animate-in fade-in slide-in-from-top-1">
              <div className="flex items-center gap-2 overflow-hidden">
                <span className={`p-1 rounded shrink-0 ${getColor(ev.type)}`}>
                  {getIcon(ev.type)}
                </span>
                <span className="font-mono text-zinc-300 text-xs truncate max-w-[120px]" title={ev.artifactId}>
                  {ev.artifactId}
                </span>
              </div>
