  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-zinc-500 max-w-2xl text-sm leading-relaxed">
        Real-time observation of artifact creation, lineage resolution, and replay
        verification.
      </p>
        ...
        {events.length === 0 ? (
          ...
          <div className="divide-y divide-zinc-800">
            {events.map((ev, i) => (
              <div
                key={`${ev.artifactId}-${i}`}
                className="p-4 flex items-start gap-4 hover:bg-zinc-800/30 transition-colors animate-in fade-in slide-in-from-top-2"
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    <h4 className="text-sm font-semibold text-white truncate pr-4">
                      {ev.type.toUpperCase()}{" "}
                      <span className="font-mono text-zinc-500 font-normal ml-2">
                        {ev.artifactId}
                      </span>
                    </h4>
                  ...
                  {ev.parentArtifactIds && ev.parentArtifactIds.length > 0 && (
                    <div className="mt-2 flex gap-2 flex-wrap">
                      {ev.parentArtifactIds.map((pid: string) => (
                        <span
                          key={pid}
                          className="text-[10px] font-mono text-zinc-500 bg-zinc-950 border border-zinc-800 px-1.5 py-0.5 rounded"
                        >
