...
export function SemanticDrift() {
    ...
    Promise.all([
      ...
      .then(([health, bundle]) => {
        ...
        const hasPartialSources =
          [bundleOk, storeOk, telOk].some(Boolean) &&
          ![bundleOk, storeOk, telOk].every(Boolean);
        ...
        setSources([
          {
            ...
            available: bundleOk,
            note: bundleOk ? bundle.source : "Not found"
          },
          {
            name: "Query Store",
            available: storeOk,
            note: storeOk ? "query-store/store.db" : "Not found"
          },
          {
            name: "Telemetry",
            available: telOk,
            note: telOk ? ".hardkas/telemetry/telemetry.jsonl" : "Not found"
          },
          {
            name: "Filesystem Artifacts",
            available: !!health.artifactsDirExists,
            note: health.artifactsDirExists ? ".hardkas/artifacts" : "Not found"
          }
        ]);
        setApiOffline(false);
        setLoading(false);
  ...
  if (loading)
    return <div className="text-zinc-500">Analyzing environment semantics...</div>;
    ...
    <div className="space-y-6">
      {apiOffline && (
        <div className="bg-red-500/10 border border-red-500/30 px-6 py-3 flex items-center gap-2 text-sm text-red-400 shrink-0 rounded-lg">
          ...
          <span>
            <strong>Observability Server Offline:</strong> Could not connect to the
            HardKAS dev-server API. Ensure the server is running by executing:{" "}
            <code className="font-mono bg-red-950/40 px-1.5 py-0.5 rounded text-red-300">
              pnpm --filter @hardkas/cli run dev dashboard
            </code>
          </span>
        </div>
      )}
      ...
      <p className="text-zinc-500 max-w-2xl text-sm leading-relaxed">
        Compares the physical Filesystem Truth against the Query-Store Projection,
        Semantic Bundle, and Telemetry. If there is any disagreement, semantic drift is
        detected.
      </p>
      ...
      {driftState === "RED" && (
        <div className="bg-red-500/10 border border-red-500/30 p-6 rounded-lg">
          ...
          <p className="text-red-400/80">
            The available truth sources are asymmetric. Some canonical stores are missing
            or disagree. HardKAS recommends reconciliation via replay or rebuild.
          </p>
        </div>
      )}
      ...
      {driftState === "YELLOW" && (
        <div className="bg-amber-500/10 border border-amber-500/30 p-6 rounded-lg">
          ...
          <p className="text-amber-400/80">
            Some truth sources are available but not all. Drift status is ambiguous — full
            verification requires all sources to be present.
          </p>
        </div>
      )}
      ...
      {driftState === "GREY" && (
        <div className="bg-zinc-500/10 border border-zinc-500/30 p-6 rounded-lg">
          ...
          <p className="text-zinc-400/80">
            Unable to determine drift status. The observability API is unreachable — no
            truth sources can be verified until the server is online.
          </p>
        </div>
      )}
        ...
        <div className="space-y-3">
          {sources.length === 0 && apiOffline && (
            <div className="text-zinc-600 text-sm italic">
              No source data available — API offline
            </div>
          )}
          {sources.map((s) => (
            <div key={s.name} className="flex items-center justify-between text-sm">
              <div className="flex items-center gap-3">
                <div
                  className={`w-2 h-2 rounded-full ${s.available ? "bg-emerald-400" : "bg-zinc-600"}`}
                />
