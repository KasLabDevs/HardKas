...
import { Package, Clock, Filter, Search, ChevronRight, Eye, AlertTriangle } from "lucide-react";
...
export function ArtifactsPage() {
  ...
  const groupedArtifacts = React.useMemo(() => {
    ...
    return Array.from(groups.entries()).map(([flowId, artifacts]) => {
      ...
      artifacts.sort((a: any, b: any) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
      
      const hasReceipt = artifacts.some((a: any) => a.schema?.startsWith("hardkas.txReceipt"));
      const hasReplay = artifacts.some((a: any) => a.schema?.startsWith("hardkas.replayReport"));
      const isCorrupted = artifacts.some((a: any) => a.kind === "CORRUPTED" || a.contentHash === "MISMATCH");
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-5 border-b border-zinc-800">
        <div>
          ...
          <p className="text-xs text-zinc-400 mt-1 leading-normal">
            HardKAS generates deterministic files representing state steps. Check hashes, causal lineage, and schemas.
          </p>
        </div>
      ...
      {isLoading ? (
        <div className="space-y-6">
          {[1, 2, 3].map((n) => (
            <div key={n} className="h-32 bg-zinc-900/20 border border-zinc-800 rounded-2xl animate-pulse" />
          ))}
        </div>
      ) : groupedArtifacts.length > 0 ? (
        <div className="space-y-6">
          {groupedArtifacts.map((group) => (
            <div key={group.flowId} className="bg-zinc-900/20 border border-zinc-800/80 rounded-2xl overflow-hidden shadow-sm">
                ...
                <div className="flex flex-col gap-1.5">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-[9px] font-mono font-bold px-2 py-0.5 rounded border uppercase tracking-wider ${
                      group.flowStatus === "VERIFIED" ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" :
                      group.flowStatus === "CORRUPTED" ? "bg-red-500/10 text-red-400 border-red-500/20" :
                      group.flowStatus === "COMPLETE" ? "bg-blue-500/10 text-blue-400 border-blue-500/20" :
                  ...
                  {group.artifacts.map((art: any, i: number) => {
                    ...
                    return (
                      <div key={art.artifactId || i} className="relative group">
                        ...
                        <div className={`bg-zinc-900/40 border ${isCorrupted ? 'border-red-900/50 bg-red-950/10' : 'border-zinc-800/60 hover:border-zinc-700/60'} rounded-xl p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 transition-all shadow-sm`}>
                          <div className="space-y-3">
                            <div className="flex items-center gap-3">
                              ...
                              <span className="text-[10px] font-mono text-zinc-500">
                                <span className="text-zinc-600">ID:</span> {art.artifactId}
                              </span>
                              {isCorrupted && (
                            ...
                            <div className="flex items-center gap-4 text-[10px] font-mono text-zinc-500">
                              {art.contentHash && (
                                <span title={art.contentHash}>hash: {truncate(art.contentHash, 6)}</span>
                              )}
          ...
          <div className="bg-red-950/20 border border-red-900/50 rounded-2xl p-8 flex flex-col items-center justify-center text-center">
            ...
            <h2 className="text-lg font-bold text-red-100 mb-2">Transactions exist without authoritative artifacts.</h2>
            <p className="text-sm text-red-300/80 mb-6 max-w-md">
              This workspace has projection drift. The SQLite cache contains transactions, but the definitive artifacts are missing from the filesystem.
            </p>
            <div className="bg-black/40 border border-red-900/30 rounded-xl p-4 w-full max-w-lg text-left">
              ...
              <code className="text-sm font-mono text-red-200 block mb-4">hardkas doctor --consistency --strict</code>
            </div>
          </div>
        ) : (
          <EmptyState
