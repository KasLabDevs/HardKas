...
export function ReplayPage() {
  ...
  const passedReplays = replays.filter(
    (r: any) =>
      r.ok || r.status?.toUpperCase() === "PASS" || r.status?.toUpperCase() === "SUCCESS"
  ).length;
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="pb-5 border-b border-zinc-800">
        ...
        <p className="text-xs text-zinc-400 mt-1 leading-normal">
          HardKAS executes transactions locally to deterministically verify that the
          computed state matches signed artifact receipts.
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <>
          {/* Main Status Block */}
          <div
            className={`p-6 border rounded-2xl backdrop-blur-md flex flex-col md:flex-row md:items-center justify-between gap-6 ${
                      ...
                      >
                        <div className="space-y-1 truncate text-left">
                          ...
                          <div className="flex items-center gap-3 text-[10px] text-zinc-500 font-mono select-none">
                            ...
                            <span>•</span>
                            <span>
                              {formatRelativeTime(rep.createdAt || rep.timestamp)}
                            </span>
                          </div>
                        </div>
                      </div>
              ...
              <div className="bg-zinc-900/20 border border-zinc-800/80 rounded-2xl overflow-hidden">
                ...
                <div className="divide-y divide-zinc-900">
                  {replays.map((rep: any, i: number) => {
                    const isSuccess =
                      rep.status?.toUpperCase() === "PASS" ||
                      rep.status?.toUpperCase() === "SUCCESS";
                    ...
                    return (
                      ...
                      >
                        <div className="space-y-1 truncate text-left">
                          ...
                          <div className="flex items-center gap-3 text-[10px] text-zinc-500 font-mono select-none">
                            <span>
                              Hash: {truncate(rep.artifactId || rep.hash || "", 5)}
                            </span>
                            <span>•</span>
                            <span>
                              {formatRelativeTime(rep.createdAt || rep.timestamp)}
                        ...
                        <div className="flex items-center gap-3">
                          <button
                            onClick={() =>
                              setSelectedTxId(selectedTxId === rep.txId ? null : rep.txId)
                            }
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="border border-zinc-800/80 rounded-xl overflow-hidden">
                            ...
                            <div className="p-4 bg-zinc-950/40 text-[10px] font-mono text-zinc-400">
                              No missing artifacts, projections, or excluded edges
                              detected.
                            </div>
                          </div>
                          ...
                          <div
                            className={`border ${mismatches.length > 0 ? "border-red-500/30" : "border-emerald-500/20"} rounded-xl overflow-hidden`}
                          >
                            <div
                              className={`${mismatches.length > 0 ? "bg-red-950/20 border-red-500/30" : "bg-emerald-950/10 border-emerald-500/20"} px-4 py-2 border-b flex justify-between items-center`}
                            >
                              <span
                                className={`text-[10px] font-bold ${mismatches.length > 0 ? "text-red-400" : "text-emerald-400"} uppercase tracking-widest`}
                              >
                                Layer 2: Deterministic
                              </span>
                              <span
                                className={`text-[10px] ${mismatches.length > 0 ? "text-red-400" : "text-emerald-400"} font-mono`}
                              >
                                {mismatches.length > 0 ? "Diverged" : "Matched"}
                              </span>
                            </div>
                            <div
                              className={`p-4 bg-zinc-950/40 text-[10px] font-mono ${mismatches.length > 0 ? "text-red-300/90" : "text-emerald-400/70"}`}
                            >
                              {mismatches.length > 0 ? (
                                <div className="space-y-3 select-all overflow-x-auto">
                                  {mismatches.map((m: any, idx: number) => (
                                    <div
                                      key={idx}
                                      className="pb-3 border-b border-red-500/10 last:pb-0 last:border-0"
                                    >
                                      <div className="flex gap-2">
                                        <span className="text-zinc-500 w-24 shrink-0">
                                          Path:
                                        </span>{" "}
                                        <span className="font-bold">{m.path}</span>
                                      </div>
                                      <div className="flex gap-2">
                                        <span className="text-zinc-500 w-24 shrink-0">
                                          Receipt:
                                        </span>{" "}
                                        <span className="bg-red-950/30 px-1 rounded text-red-200">
                                          {String(m.expected)}
                                        </span>
                                      </div>
                                      <div className="flex gap-2">
                                        ...
                                        <span className="bg-emerald-950/30 px-1 rounded text-emerald-200">
                                          {String(m.actual)}
                                        </span>
                                      </div>
                                    </div>
                          ...
                          <div className="border border-zinc-800/80 rounded-xl overflow-hidden">
                            ...
                            <div className="p-4 bg-zinc-950/40 text-[10px] font-mono text-zinc-500 space-y-1">
                              <div>
                                ~ Timestamp shifts implicitly ignored by strict
                                determinism.
                              </div>
            ...
            <div className="space-y-6">
              <div className="bg-zinc-900/35 border border-zinc-800 rounded-2xl p-6 space-y-4">
                ...
                <p className="text-xs text-zinc-400 leading-relaxed">
                  Local execution is deterministic. Every transaction must be reproducible
                  offline by recreating the state change transitions and verifying that
                  output roots are invariant.
                </p>
                ...
                <div className="pt-4 border-t border-zinc-800/60 space-y-4 text-[10px] text-zinc-500 leading-relaxed">
                  <div>
                    <strong className="text-zinc-400 font-sans block mb-1 flex items-center gap-1.5">
                      <CheckCircle2 size={12} className="text-emerald-500" /> Local
                      deterministic
                    </strong>
                    State roots and UTXOs compute identically offline. Replay guarantees
                    the artifact represents valid state transitions within the localnet
                    boundary.
                  </div>
                  <div>
                    <strong className="text-zinc-400 font-sans block mb-1 flex items-center gap-1.5">
                      <AlertOctagon size={12} className="text-amber-500" /> Consensus
                      validated (No)
                    </strong>
                    Replay does NOT mean the transaction is finalized on the Kaspa
                    network. It only guarantees local determinism. Network finality
                    requires a separate consensus check.
                  </div>
                  <div>
                    <strong className="text-zinc-400 font-sans block mb-1 flex items-center gap-1.5">
                      <Info size={12} className="text-sky-500" /> Replay Scope Assumptions
                    </strong>
                    - The local state projection was accurate at execution time.
                    <br />
                    - All dependency artifacts exist and match their hashes.
                    <br />- Signature checks (if any) are valid within the testnet
                    environment.
                  </div>
                  <div>
                    <strong className="text-zinc-400 font-sans block mb-1">
          ...
          {overview?.counts && overview.counts.artifacts > 0 ? (
            <div className="bg-amber-950/20 border border-amber-900/50 rounded-2xl p-8 max-w-2xl mx-auto text-center space-y-4">
              ...
              <p className="text-sm text-amber-400/80">
                Artifacts detected (e.g. tx plan, signed tx).
                <br />
              ...
              {overview?.guarantees && (
                <div className="grid grid-cols-2 gap-4 mt-6 pt-6 border-t border-amber-900/30 text-left">
                  <div>
                    <div className="text-[9px] uppercase tracking-wider text-amber-500/60">
                      Artifact Integrity
                    </div>
                    <div className="text-xs font-mono font-bold text-emerald-400">
                      {overview.guarantees.artifactIntegrity}
                    </div>
                  </div>
                  <div>
                    <div className="text-[9px] uppercase tracking-wider text-amber-500/60">
                      Local Replay Reproducibility
                    </div>
                    <div className="text-xs font-mono font-bold text-amber-400">
                      {overview.guarantees.localReplay}
                    </div>
                  </div>
                </div>
              )}
