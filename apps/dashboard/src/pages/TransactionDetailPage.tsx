      ...
      <div className="space-y-6 py-8">
        ...
        <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-6 text-center max-w-xl mx-auto my-8 space-y-3">
          ...
          <p className="text-xs text-zinc-400 leading-normal">
            The workspace was unable to resolve a transaction artifact with the identifier
            "{id}". It may have been cleaned up or re-indexed.
          </p>
        </div>
      </div>
    );
      ...
      <div className="bg-zinc-900/40 border border-zinc-800 rounded-2xl p-6 backdrop-blur-md space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 self-start md:self-center">
            <span
              className={`inline-flex items-center px-2.5 py-0.5 rounded text-xs font-bold font-mono border uppercase tracking-wider ${
                isPlan
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-zinc-900/30 border border-zinc-800 rounded-2xl p-6 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[9px] font-mono bg-zinc-950 px-1.5 py-0.5 rounded border border-zinc-850 text-zinc-400">
              OUTGOING
            </span>
          </div>
          ...
          <div className="font-mono text-xs text-zinc-300 bg-zinc-950/40 border border-zinc-900/60 p-3 rounded-xl flex items-center justify-between gap-3 truncate hover:border-zinc-800">
            <span className="truncate select-all" title={tx.from}>
              {tx.from || "—"}
            </span>
            {tx.from && (
              <button
        ...
        <div className="bg-zinc-900/30 border border-zinc-800 rounded-2xl p-6 space-y-4">
          ...
          <div className="font-mono text-xs text-zinc-300 bg-zinc-950/40 border border-zinc-900/60 p-3 rounded-xl flex items-center justify-between gap-3 truncate hover:border-zinc-800">
            <span className="truncate select-all" title={tx.to}>
              {tx.to || "—"}
            </span>
            {tx.to && (
              <button
        ...
        nodes={[
          {
            ...
            label:
              (tx.rawArtifact?.artifactId || tx.txId || "").slice(0, 8) || "Artifact",
            status:
              tx.rawArtifact?.contentHash === "INVALID_JSON" ||
              tx.rawArtifact?.kind === "CORRUPTED" ||
              tx.rawArtifact?.contentHash === "MISMATCH"
                ? "error"
                : "ok",
          ...
          },
          {
            ...
            status:
              tx.replayStatus === "SUCCESS" || tx.replayStatus === "PASS"
                ? "ok"
                : tx.replayStatus
                  ? "error"
                  : "warn",
