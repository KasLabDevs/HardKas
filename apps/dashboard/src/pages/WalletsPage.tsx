...
function SandboxQR({ uri }: { uri: string }) {
  return (
    <div className="bg-white p-3 rounded-xl inline-block shadow-lg animate-in fade-in zoom-in duration-300">
      <QRCode value={uri} size={140} bgColor="#ffffff" fgColor="#0d0d12" level="Q" />
      <div className="text-center mt-2 w-[140px] truncate">
        <span className="text-[7px] text-zinc-500 break-all leading-tight font-mono">
          {uri}
        </span>
      </div>
    </div>
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="pb-5 border-b border-zinc-800">
        ...
        <p className="text-xs text-zinc-400 mt-1 leading-normal">
          Manage local adapter injections for KasWare (L1) and MetaMask (L2), or run
          WalletConnect simulations in the sandbox.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <section className="bg-zinc-900/40 border border-zinc-800 rounded-2xl p-6 flex flex-col justify-between">
          <div className="space-y-4">
            ...
            <p className="text-xs text-zinc-400 leading-relaxed">
              Synchronize the injected KasWare wallet with your deterministic L1 dev
              session to mock key signs.
            </p>
            ...
            <div className="space-y-3 pt-2">
              ...
              {kwState.connected ? (
                <div
                  className="bg-zinc-950/40 border border-zinc-900 p-3 rounded-xl font-mono text-[10px] text-zinc-300 truncate select-all"
                  title={kwState.address}
                >
                  {kwState.address}
                </div>
              ) : kwState.installed ? (
                ...
                <div className="text-[10px] text-red-400/80 italic leading-normal p-3 rounded-xl bg-red-950/5 border border-red-500/10">
                  Extension not detected. Please install the KasWare Chrome extension to
                  enable L1 wallet sync.
                </div>
              )}
            </div>
          </div>
        ...
        <section className="bg-zinc-900/40 border border-zinc-800 rounded-2xl p-6 flex flex-col justify-between">
          <div className="space-y-4">
            ...
            <p className="text-xs text-zinc-400 leading-relaxed">
              Connect MetaMask to the local Igra L2 rollup chain and ensure L2 session
              alignment.
            </p>
            ...
            <div className="space-y-3 pt-2">
              ...
              {mmState.connected ? (
                <div
                  className={`p-3 rounded-xl font-mono text-[10px] border ${
                    mmSessionMatch
                      ? "bg-zinc-950/40 border-zinc-900 text-emerald-400"
                      : "bg-orange-950/5 border-orange-500/10 text-orange-400"
                  }`}
                >
                  <div className="truncate select-all" title={mmState.account}>
                    Address: {mmState.account}
                  </div>
                  ...
                  {!mmSessionMatch && (
                    <div className="text-[8px] text-orange-400/90 italic leading-tight mt-2 border-t border-orange-500/10 pt-1.5">
                      → Switch your active MetaMask account to match expected:{" "}
                      {session?.l2.address}
                    </div>
                  )}
                </div>
                ...
                <div className="text-[10px] text-red-400/80 italic leading-normal p-3 rounded-xl bg-red-950/5 border border-red-500/10">
                  MetaMask not found. Please install the MetaMask Chrome extension to
                  enable L2 wallet sync.
                </div>
              )}
            </div>
          </div>
                ...
                >
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3">
                      <span
                        className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[9px] font-mono font-bold uppercase border ${
                          s.status === "paired"
                            ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20"
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-6 justify-center py-4 bg-zinc-950/20 border border-zinc-900 rounded-xl">
                      ...
                      <div className="space-y-3 text-center sm:text-left max-w-xs">
                        ...
                        <p className="text-[10px] text-zinc-500 leading-relaxed">
                          Scan this QR using a compatible mock mobile wallet, or bypass
                          mobile completely by triggering a local pair simulator.
                        </p>
                        <button
                  ...
                  {s.status === "paired" && (
                    <div className="space-y-3 bg-zinc-950/20 border border-zinc-900 p-4 rounded-xl font-mono text-[10px] animate-in slide-in-from-bottom-2">
                      <div className="flex justify-between gap-3">
                        ...
                        <span className="text-zinc-300 select-all truncate max-w-xs">
                          {s.l1Address || "None"}
                        </span>
                      </div>
                      <div className="flex justify-between gap-3">
          ...
          <div className="bg-zinc-950/20 border border-zinc-900 rounded-2xl p-6 flex flex-col justify-between">
            <div className="space-y-4">
              ...
              <ul className="space-y-3.5 text-[10px] text-zinc-500 leading-relaxed font-sans">
                <li>
                  • Ephemeral simulations: No real network traffic leaves your local
                  machine context.
                </li>
                <li>
                  • Pairing triggers session state: Pairing a sandbox utilizes session
                  wallets to bypass authentic approvals.
                </li>
                <li>
                  • Dev testing cockpit: Use this sandbox to experiment with QR-based
                  onboarding and mock sign notifications in your apps.
                </li>
              </ul>
            </div>

            <div className="mt-8 pt-4 border-t border-zinc-900">
              <span className="text-[8px] uppercase font-mono text-indigo-400 tracking-widest block mb-1">
                Local URI Schema
              </span>
