      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-5 border-b border-zinc-800">
        <div>
          ...
          <p className="text-xs text-zinc-400 mt-1 leading-normal">
            Cockpit visualizes all offline transaction plans, signed payloads, and
            broadcasted receipts in chronological order.
          </p>
        </div>
      ...
      {isLoading ? (
        <div className="space-y-3">
          {[1, 2, 3].map((n) => (
            <div
              key={n}
              className="h-14 bg-zinc-900/30 border border-zinc-800 rounded-xl animate-pulse"
            />
          ))}
        </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-mono font-bold border uppercase tracking-wider ${
                            isPlan
                      ...
                      <td className="px-6 py-4 font-mono font-bold text-zinc-200">
                        <div className="flex flex-col gap-1">
                          {tx.txId ? (
                            <span
                              className="text-xs font-bold text-zinc-200 tracking-wide select-all"
                              title={tx.txId}
                            >
                              {truncate(tx.txId, 6)}
                            </span>
                      ...
                      <td className="px-6 py-4 font-mono font-extrabold text-zinc-200">
                        <span className="text-zinc-200">
                          {formatIgraBalance(
                            safeBigInt(tx.amount || tx.amountSompi),
                            tx.from
                          )}
                        </span>
                        <span className="text-[9px] text-zinc-500 font-bold ml-1.5 uppercase">
                          {tx.from?.startsWith("kaspa:") ? "KAS" : "iKAS"}
                        </span>
                      </td>
        ...
        <div className="bg-amber-950/20 border border-amber-900/50 rounded-2xl p-8 flex flex-col items-center justify-center text-center">
          ...
          <h2 className="text-lg font-black text-amber-100 mb-2 tracking-tight">
            No Transaction Receipts Indexed
          </h2>
          <p className="text-sm text-amber-400/80 max-w-md mb-6 leading-relaxed">
            Runtime artifacts exist, but none of them are transaction receipts. The
            existing artifacts are likely just plans or unsigned envelopes.
          </p>
