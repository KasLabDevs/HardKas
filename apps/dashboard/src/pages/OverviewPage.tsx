...
function formatIgraBalance(
  amount: bigint | undefined,
  address: string | undefined
): string {
  if (amount === undefined) return "0.00000000";
  const isSimulated = address
    ? address.startsWith("0xsim_") ||
      address.startsWith("kaspa:sim_") ||
      ...
      <div className="bg-zinc-900/60 border border-zinc-800/80 rounded-2xl overflow-hidden shadow-md">
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="flex flex-col gap-3">
              <div className="bg-zinc-950/50 border border-zinc-800 rounded-xl p-3 flex items-start gap-3">
                ...
                <div>
                  ...
                  <div
                    className="text-[10px] font-mono text-zinc-500 mt-0.5 truncate max-w-[250px]"
                    title={overview?.workspaceRoot}
                  >
          ...
          <div className="space-y-4">
            ...
            <div
              className={`border rounded-xl p-4 flex flex-col h-[calc(100%-28px)] justify-between ${
                overview?.runtimeState === "EMPTY"
                  ? "bg-zinc-950/50 border-zinc-800"
                  : overview?.runtimeState === "ACTIVE"
                    ? "bg-blue-950/20 border-blue-900/50"
                    : overview?.runtimeState === "PENDING"
                      ? "bg-amber-950/20 border-amber-900/50"
                      : overview?.runtimeState === "DEGRADED"
                        ? "bg-orange-950/20 border-orange-900/50"
                        : overview?.runtimeState === "CORRUPTED"
                          ? "bg-red-950/20 border-red-900/50"
                          : overview?.runtimeState === "VERIFIED"
              ...
              {overview?.guarantees && (
                <div className="mt-4 grid grid-cols-2 gap-2 pt-4 border-t border-zinc-800/50">
                  <div>
                    ...
                    <div
                      className={`text-xs font-mono font-bold ${overview.guarantees.artifactIntegrity === "available" ? "text-emerald-400" : overview.guarantees.artifactIntegrity === "failed" ? "text-red-400" : "text-zinc-400"}`}
                    >
                  ...
                  <div>
                    ...
                    <div
                      className={`text-xs font-mono font-bold ${overview.guarantees.localReplay === "verified" ? "text-emerald-400" : overview.guarantees.localReplay === "failed" ? "text-red-400" : "text-zinc-400"}`}
                    >
              ...
              {session ? (
                <div className="space-y-5 flex-1 flex flex-col justify-center">
                  <div>
                    ...
                    <div
                      className="font-mono text-xs text-indigo-300 bg-zinc-950/50 border border-zinc-900/80 p-3 rounded-xl mt-2 truncate hover:border-zinc-800 transition-all"
                      title={session.l1.address}
                    >
                  ...
                  <div>
                    ...
                    <div
                      className="font-mono text-xs text-purple-300 bg-zinc-950/50 border border-zinc-900/80 p-3 rounded-xl mt-2 truncate hover:border-zinc-800 transition-all"
                      title={session.l2.address}
                    >
              ...
              <div className="space-y-5 flex-1 flex flex-col justify-center">
                <div className="flex items-center justify-between pb-4 border-b border-zinc-850">
                  <div className="flex flex-col">
                    ...
                    <span className="text-2xl font-black font-mono text-zinc-100 mt-2">
                      {health?.kaspa?.status === "offline"
                        ? "—"
                        : kaspaBalance !== undefined
                          ? formatSompi(kaspaBalance)
                          : "0.00000000"}
                    </span>
                  </div>
                  <span className="text-xs font-black font-mono text-indigo-400 uppercase tracking-widest bg-indigo-500/5 border border-indigo-500/10 px-2 py-0.5 rounded-md">
                    KAS
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <div className="flex flex-col">
                    ...
                    <span className="text-2xl font-black font-mono text-zinc-100 mt-2">
                      {health?.igra?.status === "offline"
                        ? "—"
                        : igraBalance !== undefined
                          ? formatIgraBalance(igraBalance, session?.l2?.address)
                          : "0.00000000"}
                    </span>
                  </div>
                  <span className="text-xs font-black font-mono text-purple-400 uppercase tracking-widest bg-purple-500/5 border border-purple-500/10 px-2 py-0.5 rounded-md">
                    iKAS
                  </span>
                </div>
              </div>
            ...
            <div className="space-y-5 font-sans">
              {healthLoading ? (
                <div className="text-xs text-zinc-500 italic animate-pulse">
                  Syncing service health...
                </div>
              ) : healthError ? (
                <div className="text-xs text-red-400 font-medium">
                  Failed to load local RPC states. Server offline.
                </div>
              ) : (
                <>
                  {/* Kaspa L1 Node */}
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl bg-zinc-950/30 border border-zinc-900 gap-3 hover:border-zinc-800 hover:bg-zinc-950/50 transition-all duration-200">
                    <div className="flex flex-col">
                      <div className="text-xs font-black text-zinc-200 tracking-wide">
                        Kaspa L1 Node (Localnet)
                      </div>
                    ...
                    <div className="shrink-0 flex items-center justify-start sm:justify-end">
                      <StatusBadge
                        status={health?.kaspa?.status || health?.l1?.status || "offline"}
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl bg-zinc-950/30 border border-zinc-900 gap-3 hover:border-zinc-800 hover:bg-zinc-950/50 transition-all duration-200">
                    ...
                    <div className="shrink-0 flex items-center justify-start sm:justify-end">
                      <StatusBadge
                        status={health?.igra?.status || health?.l2?.status || "offline"}
                      />
                    </div>
                  </div>
                </>
