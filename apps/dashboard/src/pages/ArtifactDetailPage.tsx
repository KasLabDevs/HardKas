...
export function ArtifactDetailPage() {
  ...
  if (error || !art) {
    return (
      <div className="space-y-6 py-8">
        <Link to="/artifacts" className="inline-flex items-center gap-2 text-xs text-zinc-400 hover:text-zinc-200">
  ...
  const getSchemaLabel = (schema: string): string => {
    switch (schema) {
      case "hardkas.txPlan": return "TX PLAN";
      case "hardkas.txReceipt": return "TX RECEIPT";
      case "hardkas.replay": return "REPLAY RESULT";
      case "hardkas.deployment": return "DEPLOYMENT";
      default: return schema.toUpperCase().replace("HARDKAS.", "");
    }
  };
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="bg-zinc-900/40 border border-zinc-800 rounded-2xl p-6 backdrop-blur-md space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-2">
            <span className="text-[9px] uppercase tracking-widest text-zinc-500 font-mono">Artifact ID</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-6 border-t border-zinc-800/60">
          <div>
            <span className="text-[9px] font-mono uppercase tracking-widest text-zinc-500">Integrity Check</span>
          ...
          <div>
            <span className="text-[9px] font-mono uppercase tracking-widest text-zinc-500">Content Hash</span>
            <div className="flex items-center gap-2 mt-1 group/hash">
              <span className="text-xs font-mono text-zinc-300 truncate max-w-[150px] select-all" title={art.contentHash || art.hash}>
              ...
              {(art.contentHash || art.hash) && (
                ...
                >
                  {copiedText === "hash" ? <Check size={10} className="text-emerald-400" /> : <Copy size={10} />}
                </button>
              )}
            </div>
          </div>
          ...
          <div>
            <span className="text-[9px] font-mono uppercase tracking-widest text-zinc-500">Last Synced</span>
      ...
      <ProvenanceGraph nodes={[
        {
          type: "artifact",
          id: art.artifactId,
          label: (art.artifactId || "").length > 8 ? `${art.artifactId.slice(0, 8)}...` : art.artifactId || "Artifact",
      ...
      {(art.lineage?.parentId || art.lineage?.rootId) && (
        <div className="bg-zinc-900/30 border border-zinc-800 rounded-2xl p-6 space-y-4">
          <h4 className="text-xs font-bold uppercase tracking-wider text-zinc-400 font-sans">Lineage Connections</h4>
          <div className="space-y-3 font-mono text-xs">
            {art.lineage?.parentId && (
              <div className="flex flex-col sm:flex-row sm:items-center justify-between p-3 rounded-xl bg-zinc-950/20 border border-zinc-900 gap-2">
                <span className="text-zinc-500 text-[10px] uppercase tracking-widest">Parent Artifact</span>
                <Link to={`/artifacts/${art.lineage.parentId}`} className="text-indigo-400 hover:text-indigo-300 truncate max-w-lg select-all">
                  {art.lineage.parentId}
                </Link>
              </div>
            )}
            {art.lineage?.rootId && (
              <div className="flex flex-col sm:flex-row sm:items-center justify-between p-3 rounded-xl bg-zinc-950/20 border border-zinc-900 gap-2">
                <span className="text-zinc-500 text-[10px] uppercase tracking-widest">Root Artifact</span>
                <Link to={`/artifacts/${art.lineage.rootId}`} className="text-indigo-400 hover:text-indigo-300 truncate max-w-lg select-all">
                  {art.lineage.rootId}
                </Link>
              </div>
            )}
