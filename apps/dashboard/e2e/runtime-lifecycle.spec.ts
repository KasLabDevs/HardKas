...
test.describe("HardKAS P1.11 - Semantic Runtime States", () => {
  ...
  test.beforeEach(async () => {
    ...
    devServerProcess = spawn(
      npxCommand,
      ["tsx", CLI_PATH, "dev", "server", "--port", port.toString()],
      {
        cwd: workspaceRoot,
  ...
  test("Test A: Clean Simulated Runtime (ACTIVE -> VERIFIED)", async ({ page }) => {
    ...
    execSync("npx tsx " + CLI_PATH + " accounts fund alice --amount 1000", {
      cwd: workspaceRoot,
      stdio: "ignore"
    });
    ...
    execSync(
      "npx tsx " +
        CLI_PATH +
        " tx send --network simulated --from alice --to bob --amount 10 --yes",
    ...
    if (planFile) {
      fs.copyFileSync(
        ...
        path.join(workspaceRoot, "tx-plan.json")
      );
    }
    if (receiptFile) {
      fs.copyFileSync(
        path.join(artifactsDir, receiptFile),
        path.join(workspaceRoot, "tx-receipt.json")
      );
    }

    // Force a query store rebuild so cache aligns immediately
    execSync("npx tsx " + CLI_PATH + " query store rebuild", {
      cwd: workspaceRoot,
      stdio: "ignore"
    });
    ...
    // Run replay verify
    execSync("npx tsx " + CLI_PATH + " replay verify", {
      cwd: workspaceRoot,
      stdio: "ignore"
    });
    ...
    await expect(
      page.getByText("Local deterministic runtime is consistent")
    ).toBeVisible();
  ...
  test("Test B: Corrupted Artifact Runtime (CORRUPTED)", async ({ page }) => {
    ...
    await expect(
      page.getByText("Artifact integrity failed. Deterministic replay is unsafe.")
    ).toBeVisible();

    // Replay Page should show replay candidates excluded or fail state
    await page.goto(baseUrl + "/replay");
    await expect(
      page.getByText("No replay candidates yet").or(page.getByText("failed"))
    ).toBeVisible();

    // Strict doctor check should fail (using correct root command with --consistency)
    expect(() => {
      execSync("npx tsx " + CLI_PATH + " doctor --consistency --strict", {
        cwd: workspaceRoot,
        stdio: "pipe"
      });
    }).toThrow();
  ...
  test("Test C: Missing Projection (DEGRADED)", async ({ page }) => {
    ...
    await expect(
      page.getByText("Projection cache (SQLite) is missing or out of sync")
    ).toBeVisible();
