...
describe("P52 Full Docker Runtime Gauntlet", () => {
    ...
    beforeAll(async () => {
        try {
            ...
            await fs.writeFile(path.join(process.cwd(), "../../P52_BLOCKED_DOCKER_UNAVAILABLE.md"), "# P52 BLOCKED\nDocker is unavailable on this host.");
        }
        if (dockerAvailable) {
    ...
    it("should run the full gauntlet if Docker is available", async () => {
        ...
        try {
            ...
            await fs.writeFile(path.join(process.cwd(), "../../docker-runtime-gauntlet.evidence.json"), JSON.stringify(evidence, null, 2));
        }
        catch (e) {
            console.error(e);
            throw e;
...
//# sourceMappingURL=full-docker-gauntlet.test.js.map