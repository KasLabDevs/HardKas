...
async function main() {
  ...
  const sdk = await Hardkas.open({ cwd: process.cwd(), network: "simnet" });
  ...
  await sdk.artifacts.write(planResult as any);

  console.log("7. Signing transaction...");
  const signed = await sdk.tx.sign(planResult);
  await sdk.artifacts.write(signed as any);
  ...
  console.log(`TxID: ${(receipt as any).txId}`);
