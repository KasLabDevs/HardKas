  ...
  for (const flow of flows) {
    ...
    if (flow.sdk) {
      ...
      sdkGaps.push({
        ...
        reason:
          "CLI flow exists but no equivalent high-level SDK API was found in 0.10.0-alpha."
      });
    }
    parityResults.push({
      flow: flow.name,
...
function writeReports() {
  ...
  const result = {
    ...
    parityResults,
    notes,
    recommendedNextRelease: "0.10.0-alpha"
  };
  ...
  const md = `# Post-Release Findings For 0.10.0-alpha
...
- Resolved 0.10.0-alpha findings: ${resolvedFindings.length}
