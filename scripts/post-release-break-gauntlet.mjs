  ...
  for (const flow of flows) {
    ...
    if (flow.sdk) {
      ...
      sdkGaps.push({
        ...
        reason:
          "CLI flow exists but no equivalent high-level SDK API was found in 0.11.6-alpha."
      });
    }
    parityResults.push({
      flow: flow.name,
...
function writeReports() {
  ...
  const result = {
    ...
    parityResults,
    notes,
    recommendedNextRelease: "0.11.6-alpha"
  };
  ...
  const md = `# Post-Release Findings For 0.11.6-alpha
...
- Resolved 0.11.6-alpha findings: ${resolvedFindings.length}
...
## Recommended 0.11.6-alpha Backlog
