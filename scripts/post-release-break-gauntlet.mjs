  ...
  for (const flow of flows) {
    ...
    if (flow.sdk) {
      ...
      sdkGaps.push({
        ...
        reason:
          "CLI flow exists but no equivalent high-level SDK API was found in 0.12.0-rc.19."
      });
    }
    parityResults.push({
      flow: flow.name,
...
function writeReports() {
  ...
  const result = {
    ...
    parityResults,
    notes,
    recommendedNextRelease: "0.12.0-rc.19"
  };
  ...
  const md = `# Post-Release Findings For 0.12.0-rc.19
...
- Resolved 0.12.0-rc.19 findings: ${resolvedFindings.length}
