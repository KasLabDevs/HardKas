  ...
  for (const flow of flows) {
    ...
    if (flow.sdk) {
      ...
      sdkGaps.push({
        ...
        reason:
          "CLI flow exists but no equivalent high-level SDK API was found in 0.12.0-rc.10."
      });
    }
    parityResults.push({
      flow: flow.name,
...
function writeReports() {
  ...
  const result = {
    ...
    parityResults,
    notes,
    recommendedNextRelease: "0.12.0-rc.10"
  };
  ...
  const md = `# Post-Release Findings For 0.12.0-rc.10
...
- Resolved 0.12.0-rc.10 findings: ${resolvedFindings.length}
...
## Recommended 0.12.0-rc.10 Backlog
