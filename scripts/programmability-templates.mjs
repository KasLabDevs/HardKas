...
for (const template of templates) {
  for (const file of ["README.md", "main.ts"]) {
    ...
    if (!fs.existsSync(target)) missing.push(path.relative(root, target).replace(/\\/g, "/"));
  }
}
...
const result = {
  ...
  status: missing.length === 0 ? "PROGRAMMABILITY_TEMPLATES_READY" : "PROGRAMMABILITY_TEMPLATES_INCOMPLETE",
