...
export async function runCoverageMode(targetVersion = "0.7.3-alpha") {
  console.log("\n============================================================================");
  ...
  console.log(`📦 Setting up clean workspace and installing @hardkas/cli@${targetVersion}...`);
  ...
  execSync(`${npmCommand} install --no-audit --no-fund --legacy-peer-deps`, { cwd: workspaceDir, stdio: "ignore" });
  
  // Initialize HardKAS to make local commands happy
  console.log(`   - Initializing HardKAS project...`);
  execSync(`${npxCommand} hardkas init . --force`, { cwd: workspaceDir, stdio: "ignore" });
  ...
  const staticRootCommands = [
    "init", "up", "tx", "artifact", "replay", "rpc", "dag", "accounts", 
    "node", "config", "query", "test", "doctor", "verify", "verify-semantics", 
    "rebuild", "run", "lock", "console", "localnet", "deploy", "dev", 
    "local", "kaspa", "dashboard", "explain", "torture", "telemetry", 
    "repair", "rotate", "inspect", "chaos", "status", "why", "ci", "sandbox", "workflow"
  ];
  ...
  function shallowDiscover(baseCmd: string) {
    ...
    let helpOutput = "";
    try {
      helpOutput = execSync(`${npxCommand} ${baseCmd} --help`, { cwd: workspaceDir, encoding: "utf8" });
    } catch (e: any) {
      helpOutput = e.stdout?.toString() || e.message;
      commandMatrix[baseCmd] = {
        id: baseCmd, isL2: false, mutatesFs: false, requiresNetwork: false,
        helpCovered: false, validSmokeCovered: false, invalidSmokeCovered: false,
        excludedReason: "CRASH ON --help", flagsDiscovered: []
      };
      return;
    }
    ...
    for (const line of lines) {
      if (line.match(/^Options:/)) { parsingFlags = true; continue; }
      if (line.match(/^Commands:/)) { parsingFlags = false; continue; }
      if (parsingFlags) {
    ...
    const isL2 = baseCmd.includes("l2") || baseCmd.includes("artifact") || baseCmd.includes("replay") || baseCmd.includes("workflow");
    const requiresNetwork = baseCmd.includes("tx") || baseCmd.includes("rpc") || baseCmd.includes("dag") || baseCmd.includes("accounts fund");
    const mutatesFs = baseCmd.includes("init") || baseCmd.includes("accounts fund") || baseCmd.includes("accounts import") || baseCmd.includes("repair");

    commandMatrix[baseCmd] = {
      id: baseCmd,
      isL2, mutatesFs, requiresNetwork,
      helpCovered: true,
  ...
  const knownDeep = [
    "hardkas accounts list", "hardkas accounts fund", "hardkas accounts import", "hardkas accounts export", "hardkas accounts balance",
    "hardkas query artifacts", "hardkas query store rebuild", "hardkas query lineage",
    "hardkas dev server", "hardkas dev tools",
    "hardkas artifact inspect", "hardkas artifact push",
    "hardkas tx plan", "hardkas tx simulate", "hardkas tx sign", "hardkas tx broadcast"
  ];
  ...
  for (const [cmd, metrics] of Object.entries(commandMatrix)) {
    ...
    try {
      execSync(`${npxCommand} ${cmd} --invalid-flag-12345`, { cwd: workspaceDir, stdio: "ignore" });
    ...
    if (cmd === "hardkas console" || cmd === "hardkas dev server" || cmd === "hardkas dashboard" || cmd === "hardkas sandbox") {
    ...
    try {
      execSync(`${npxCommand} ${cmd}`, { cwd: workspaceDir, stdio: "ignore", timeout: 10000 });
  ...
  const devServerProcess = spawn(npxCommand, ["hardkas", "dev", "server", "--port", port.toString(), "--open", "false"], {
    cwd: workspaceDir,
  ...
  try {
    ...
    for (let i = 0; i < 50; i++) {
      try {
        const res = await fetch(`http://localhost:${port}/api/health`, { headers: { "Authorization": `Bearer ${token}` } });
    ...
    if (isReady) {
      ...
      const playwrightPath = pathToFileURL(path.join(workspaceDir, "node_modules", "playwright")).href;
  ...
  // Dump JSON matrices
  fs.writeFileSync(path.join(tempDirParent, "command-matrix.json"), JSON.stringify(commandMatrix, null, 2));
  fs.writeFileSync(path.join(tempDirParent, "api-matrix.json"), JSON.stringify(Array.from(apiEndpoints), null, 2));
  fs.writeFileSync(path.join(tempDirParent, "dashboard-route-matrix.json"), JSON.stringify(Array.from(dashboardRoutes), null, 2));
...
${Array.from(apiEndpoints).map(e => `- \`${e}\``).join("\n") || "- None"}

## Dashboard Routes Visited
${Array.from(dashboardRoutes).map(e => `- \`${e}\``).join("\n")}
...
`;
  ...
  console.log(`📊 Help Pct: ${helpPct.toFixed(1)}%, Invalid Pct: ${invalidPct.toFixed(1)}%`);
