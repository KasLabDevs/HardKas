...
async function extract() {
  ...
  const sdkPkg = JSON.parse(
    fs.readFileSync(path.join(hardkasDir, "packages/sdk/package.json"), "utf8")
  );
  const cliPkg = JSON.parse(
    fs.readFileSync(path.join(hardkasDir, "packages/cli/package.json"), "utf8")
  );

  // Read core exports
  const corePkg = JSON.parse(
    fs.readFileSync(path.join(hardkasDir, "packages/core/package.json"), "utf8")
  );
