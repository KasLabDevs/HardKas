...
if (!useLegacy) {
  ...
  console.log(
    "\x1b[2m  HardKAS SilverScript lifecycle remains limited to testnet/simnet.\x1b[0m"
  );
...
async function main() {
  if (!buildFromSource) {
    ...
    try {
      ...
      try {
        execSync(
          `docker run --rm ${TOCCATA_IMAGE} kaspad ${KASPAD_NETWORK_ARGS.join(" ")} --version`,
          { stdio: "pipe" }
        );
      } catch (err) {
        ...
        if (
          ...
          (out.includes("unexpected value") && out.includes("--testnet"))
        ) {
          console.log(
            `  \x1b[31m❌\x1b[0m Network args [${KASPAD_NETWORK_ARGS.join(" ")}] are not supported by this image.`
          );
      ...
      const res = execSync(`docker ${dockerArgs.join(" ")}`)
        .toString()
        .trim();
    ...
    console.log(
      `[1] Building rusty-kaspa from source (tag: \x1b[36m${TOCCATA_VERSION}\x1b[0m)`
    );
    ...
    if (!fs.existsSync(repoDir)) {
      ...
      try {
        execSync(`git clone https://github.com/kaspanet/rusty-kaspa.git ${repoDir}`, {
          stdio: "inherit"
        });
      } catch (e) {
    ...
    try {
      ...
      const child = spawn(kaspadPath, [...KASPAD_NETWORK_ARGS, "--utxoindex"], {
        stdio: "pipe"
      });
