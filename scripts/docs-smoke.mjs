...
async function run() {
  ...
  for (const file of files) {
    ...
    // Check JS/TS blocks
    const js = extractCodeBlocks(content, 'javascript').concat(extractCodeBlocks(content, 'typescript'));
    ...
    for (const code of bash) {
      ...
      for (const line of lines) {
        if (line.startsWith('hardkas ')) {
          ...
          const knownCommands = ['tx', 'accounts', 'query', 'artifact', 'dev', 'telemetry', 'doctor', 'status', 'verify-semantics', 'localnet', 'init'];
