...
fs.writeFileSync(
  ...
  JSON.stringify({ name: "discovery-lab", type: "module" })
);
console.log("Installing @hardkas/cli@0.7.11-alpha and @hardkas/sdk@0.7.11-alpha...");
execSync("npm install @hardkas/cli@0.7.11-alpha @hardkas/sdk@0.7.11-alpha", {
  cwd: workspace,
  stdio: "ignore"
});
...
function classifyCommand(cmdPath, flags) {
  ...
  if (
    pathStr.includes("send") ||
    pathStr.includes("sign") ||
    pathStr.includes("append")
  ) {
    classification.dangerous = true;
  }
  if (
    pathStr.includes("rpc") ||
    pathStr.includes("network") ||
    pathStr.includes("sync")
  ) {
    classification.requiresNetwork = true;
  }
  if (
    pathStr.includes("verify") ||
    pathStr.includes("simulate") ||
    pathStr.includes("receipt") ||
    pathStr.includes("send") ||
    pathStr.includes("append") ||
    pathStr.includes("sign")
  ) {
  ...
  for (const [namespace, methods] of Object.entries(parsedSdk)) {
    if (Array.isArray(methods)) {
      ...
      sdkApis.push({
        namespace: "root",
        method: namespace,
        fullPath: `sdk.${namespace}`
      });
    }
  }
} catch (e) {
  console.log(
    "Failed to dynamically probe SDK, will fall back to static list or partial execution."
  );
}
...
fs.writeFileSync(
  path.join(reportsDir, "full-command-surface.json"),
