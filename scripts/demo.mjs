...
const dryResult = run(nodeCmd, [hardkasBin, "workflow", "run", "demo-transfer", "--dry-run", "--json"], { expectJson: true });

// 5. Real workflow execution
console.log("\n── Step 3: Workflow Execute ──");
const execResult = run(nodeCmd, [hardkasBin, "workflow", "run", "demo-transfer", "--json"], { expectJson: true });
...
const rebuildResult = run(nodeCmd, [hardkasBin, "rebuild", "--from-artifacts", "--json"], { expectJson: true });
...
console.log(`  ✔ Rebuild indexed ${rebuildResult.artifacts.indexed} artifact(s), ${rebuildResult.events.indexed} event(s).`);

// 8. Verify Lineage
console.log("\n── Step 6: Verify Integrity ──");
const verifyResult = run(nodeCmd, [hardkasBin, "verify", "--deep", "--json"], { expectJson: true });
...
console.log(`  • Rebuild after crash: ✔ (${rebuildResult.artifacts.indexed} artifacts re-indexed)`);
console.log(`  • Verify after rebuild: ✔ (${verifyResult.successCount}/${verifyResult.scanned} passed)`);
console.log("═══════════════════════════════════════════════════\n");
