...
Write-Host "=== HardKAS NPM Smoke Test (0.6.0-alpha) ==="
...
# 2. Scaffold new project using npx @hardkas/cli@0.6.0-alpha
Write-Host "`nStep 2: Scaffolding new project using NPM package..."
# We run npx @hardkas/cli@0.6.0-alpha init with --skip-install to manually install with npm afterwards
& npx @hardkas/cli@0.6.0-alpha init $ProjectName --skip-install
if ($LASTEXITCODE -ne 0) {
    throw "npx @hardkas/cli@0.6.0-alpha init failed"
}
...
if ($cliVersion -ne "0.6.0-alpha") {
    throw "Expected version 0.6.0-alpha, got $cliVersion"
}
