...
Write-Host "=== HardKAS NPM Smoke Test (0.5.5-alpha) ==="
...
# 2. Scaffold new project using npx @hardkas/cli@0.5.5-alpha
Write-Host "`nStep 2: Scaffolding new project using NPM package..."
# We run npx @hardkas/cli@0.5.5-alpha new with --skip-install to manually install with npm afterwards
& npx @hardkas/cli@0.5.5-alpha new $ProjectName --skip-install
if ($LASTEXITCODE -ne 0) {
    throw "npx @hardkas/cli@0.5.5-alpha new failed"
}
...
if ($cliVersion -ne "0.5.5-alpha") {
    throw "Expected version 0.5.5-alpha, got $cliVersion"
}
