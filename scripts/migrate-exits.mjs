...
for (const file of files) {
  ...
  content = content.replace(
    /handleLockError\(([^)]+)\);\s*process\.exitCode\s*=\s*1;/g,
    "throw $1;"
  );

  // Replace handleError + exit
  content = content.replace(
    /handleError\(([^)]+)\);\s*process\.exitCode\s*=\s*1;/g,
    "throw $1;"
  );
  ...
  content = content.replace(
    /process\.exitCode\s*=\s*1;/g,
    'throw new Error("Command failed");'
  );

  // Replace process.exit(1)
  content = content.replace(
    /process\.exit\(\s*1\s*\);/g,
    'throw new Error("Command failed");'
  );
  ...
  content = content.replace(
    /process\.exit\(ChaosExitCodes\.INTERNAL_FAILURE\);/g,
