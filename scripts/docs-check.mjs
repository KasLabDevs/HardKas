...
const rootMdFiles = ['README.md', 'RUNTIME_CONTRACT.md', 'RUNTIME_SEMANTICS.md', 'SECURITY.md', 'CONTRIBUTING.md'].map(f => path.join(projectRoot, f)).filter(f => fs.existsSync(f));
...
const CURRENT_VERSION = '0.7.10-alpha';
...
for (const file of mdFiles) {
  ...
  while ((match = linkRegex.exec(content)) !== null) {
    ...
    if (!fs.existsSync(targetPath)) {
      console.error(`\u274c [${path.relative(projectRoot, file)}] Broken link: ${linkPath}`);
      hasErrors = true;
    }
  }
}
