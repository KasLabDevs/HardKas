...
async function run() {
  ...
  const utxos = [
    { id: "utxo_1", address: "kaspa:sim_alice", amountSompi: "50000000000000", spent: false }, // 50,000 KAS
    { id: "utxo_2", address: "kaspa:sim_bob", amountSompi: "1500000000000", spent: false }, // 1,500 KAS
    { id: "utxo_3", address: "kaspa:sim_carol", amountSompi: "250000000000", spent: false }, // 250 KAS
    { id: "utxo_4", address: "kaspa:sim_dave", amountSompi: "10000000000", spent: false }, // 10 KAS
  ];
  
  fs.writeFileSync(localnetPath, JSON.stringify({
    version: "1.0.0",
    network: "simulated",
    height: 12543,
    utxos
  ...
  for (let i = 1; i <= 3; i++) {
    ...
    fs.writeFileSync(path.join(artifactsDir, `${artifactId}.json`), JSON.stringify(receipt, null, 2));
    ...
    fs.writeFileSync(path.join(artifactsDir, `${replayId}.json`), JSON.stringify(replay, null, 2));
    ...
    logEvent(txId, "transaction.planned", { from, to, amount: receipt.payload.amountSompi });
  ...
  console.log("Workspace seeded successfully with deterministic artifacts, events, and balances.");
}
