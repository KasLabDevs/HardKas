...
HardKAS `0.12.0-rc.19` introduces the strictly partitioned Execution Contract.
...
HardKAS `0.12.0-rc.19` is local-first deterministic transaction infrastructure
...
The `0.12.0-rc.19` release line includes a normalized Toccata v2 localnet flow:
...
### 0.12.0-rc.19 SDK Parity

`0.12.0-rc.19` is a SDK parity / developer experience patch. It adds high-level
...
`0.12.0-rc.19` via `SDK_SILVER_REAL_LIFECYCLE_UNSUPPORTED`; certified real
lifecycle execution remains CLI/localnet bounded.

### 0.12.0-rc.19 Programmability Builder Surface

`0.12.0-rc.19` also includes a local-only builder surface for SilverScript,
