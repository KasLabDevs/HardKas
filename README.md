...
HardKAS `0.11.3-alpha` is local-first deterministic transaction infrastructure
...
The `0.11.3-alpha` release line includes a normalized Toccata v2 localnet flow:
...
### 0.11.3-alpha SDK Parity

`0.11.3-alpha` is a SDK parity / developer experience patch. It adds high-level
...
`0.11.3-alpha` via `SDK_SILVER_REAL_LIFECYCLE_UNSUPPORTED`; certified real
lifecycle execution remains CLI/localnet bounded.

### 0.11.3-alpha Programmability Builder Surface

`0.11.3-alpha` also includes a local-only builder surface for SilverScript,
