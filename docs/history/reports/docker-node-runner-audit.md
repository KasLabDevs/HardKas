...
The HardKas Node Runner is a functional and robust tool for setting up local development environments (`simnet`) with a single command. It uses the Docker CLI deterministically and manages data persistence well via _bind mounts_.
...
HardKas uses _bind mounts_ to persist blockchain data outside the container.
