...
- **Ephemeral**: The SQLite database and telemetry logs are considered _observational_. They can be safely deleted, corrupted, or rotated.
...
HardKAS provides strict operational guarantees _only_ within supported environments. The following scenarios result in **undefined behavior** and are explicitly unsupported:
...
> _HardKAS provides deterministic local transaction workflow semantics for Kaspa (planning, artifact generation, replay, and auditing), while live consensus, finality, mempool behavior, and RPC truthfulness remain strictly external to the runtime contract._
