...
A `workflowId` represents the *causal intent* before execution.
It is generated *upfront* via a deterministic hash of the workflow specification, network capability, and policy context. Ambient variables (`Date.now()`, `Math.random()`, or UUIDs) do not participate in canonical identity generation.

### 4. Separation of Capabilities and Policies
- **Capabilities**: What the environment *can* do (e.g., Localnet access, Docker availability).
- **Policies**: What the workflow *is permitted* to do (e.g., `allowNetwork: false`, `requireDryRun: true`).
