...
```bash
hardkas accounts consolidate alice --dry-run
```
...
```bash
hardkas accounts consolidate alice --execute --yes
```

_Note: This process may take several minutes as it waits for node confirmations between chained sweeps._
