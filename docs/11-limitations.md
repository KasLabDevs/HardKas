...
HardKAS is currently **0.9.6-alpha**. These limits are part of the product
boundary, not footnotes.

## 1. Local-First Only For Now

HardKAS is optimized for `network: "simulated"`. Toccata v2 `simnet` is now the
certified local real-node baseline for 0.9.6-alpha, including Docker funding,
...
The `0.9.6-alpha` ZK corpus surface verifies local fixture coherence for proof artifacts.
...
RISC0 is inspect-only in `0.9.6-alpha`. Local receipt verification returns
