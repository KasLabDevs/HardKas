...
> [!WARNING]
> HardKAS is a local-first development environment. These contracts apply _strictly to the local environment_ (local-only scope). Mainnet compatibility, ZK bridging, and L1 verification logic are out of scope and explicitly volatile/unstable.
