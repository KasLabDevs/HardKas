...
function SendKasForm({ fromAlias }: { fromAlias: string }) {
  ...
  const { execute, loading, error, data } = useMutation((client, variables: { from: string, to: string, amountSompi: number }) => {
  ...
  return (
    <div style={{ marginTop: '20px', padding: '10px', border: '1px solid #ccc' }}>
      ...
      <form onSubmit={handleSend}>
        ...
        <button type="submit" disabled={loading}>{loading ? 'Sending...' : 'Send Simulator Tx'}</button>
      </form>
      {error && <div style={{ color: 'red' }}>Error: {JSON.stringify(error)}</div>}
      {data && <div style={{ color: 'green' }}>Success! Receipt TXID: {data.receipt.txId}</div>}
    </div>
  );
}
