...
describe("resolveFieldPath", () => {
  ...
  it("should resolve nested fields", () => {
    expect(resolveFieldPath({ from: { address: "kaspa:abc" } }, "from.address")).toBe(
      "kaspa:abc"
    );
  });
...
describe("evaluateFilter", () => {
  it("eq — matches string equality", () => {
    expect(
      evaluateFilter(
        ...
        { field: "schema", op: "eq", value: "hardkas.txPlan" }
      )
    ).toBe(true);
    expect(
      evaluateFilter(
        { schema: "hardkas.txPlan" },
        { field: "schema", op: "eq", value: "hardkas.signedTx" }
      )
    ).toBe(false);
  });
  ...
  it("gt/lt — numeric comparison", () => {
    expect(
      evaluateFilter({ amount: "500" }, { field: "amount", op: "gt", value: 100 })
    ).toBe(true);
    expect(
      evaluateFilter({ amount: "50" }, { field: "amount", op: "gt", value: 100 })
    ).toBe(false);
    expect(
      evaluateFilter({ amount: "50" }, { field: "amount", op: "lt", value: 100 })
    ).toBe(true);
  });
  ...
  it("in — membership check", () => {
    expect(
      evaluateFilter(
        { networkId: "simnet" },
        { field: "networkId", op: "in", value: ["simnet", "testnet"] }
      )
    ).toBe(true);
    expect(
      evaluateFilter(
  ...
  it("contains — substring match", () => {
    expect(
      evaluateFilter(
        { schema: "hardkas.txPlan" },
        { field: "schema", op: "contains", value: "txPlan" }
      )
    ).toBe(true);
    expect(
      evaluateFilter(
        { schema: "hardkas.txPlan" },
        { field: "schema", op: "contains", value: "receipt" }
      )
    ).toBe(false);
  });
  ...
  it("exists — field presence", () => {
    expect(
      evaluateFilter(
        { from: { address: "a" } },
        { field: "from", op: "exists", value: true }
      )
    ).toBe(true);
