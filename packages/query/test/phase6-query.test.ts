...
describe("EventsQueryAdapter", () => {
  ...
  it("should filter events by domain", async () => {
    ...
    expect((result.items[0] as Record<string, unknown>).kind).toBe(
...
describe("Type-aware filters", () => {
  it("should use strict equality (no String coercion)", () => {
    ...
    expect(evaluateFilter({ val: 42 }, { field: "val", op: "eq", value: "42" })).toBe(
      false
    );
    // Same type should match
    expect(
      evaluateFilter({ val: "hello" }, { field: "val", op: "eq", value: "hello" })
    ).toBe(true);
  });

  it("should return false for invalid numeric comparisons (not NaN => 0)", () => {
    expect(
      evaluateFilter({ val: "not-a-number" }, { field: "val", op: "gt", value: "10" })
    ).toBe(false);
    expect(
      evaluateFilter({ val: undefined }, { field: "val", op: "lt", value: "5" })
    ).toBe(false);
  });

  it("should handle bigint comparisons", () => {
    expect(
      evaluateFilter({ val: BigInt(1000) }, { field: "val", op: "gt", value: "500" })
    ).toBe(true);
    expect(
      evaluateFilter({ val: BigInt(100) }, { field: "val", op: "lt", value: "500" })
    ).toBe(true);
  });

  it("should handle exists filter", () => {
    expect(
      evaluateFilter({ val: "present" }, { field: "val", op: "exists", value: true })
    ).toBe(true);
    expect(evaluateFilter({}, { field: "val", op: "exists", value: true })).toBe(false);
    expect(
      evaluateFilter({ val: null }, { field: "val", op: "exists", value: true })
    ).toBe(false);
  });
});
