...
describe("CoinSelector", () => {
  ...
  it("exact match (if fees were zero, but fees exist, so needs more)", () => {
    ...
    // Mass (kaspa-wasm 2.0.1, 1 P2PK input, 1 output): 1624.
    // Fee = 162400.
    // Conservative fee = (162400 * 110 + 99)/100 = 178640n.
    // Target 1000 + 178640 = 179640n.
    const utxoExact = [createMockUtxo({ address: "kaspatest:ex", amountSompi: 179640n, index: 0 })];
    ...
    expect(result.estimatedFeeSompi).toBe(178640n);
