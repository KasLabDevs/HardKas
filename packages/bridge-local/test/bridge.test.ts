...
describe("Bridge Local", () => {
  describe("Payload Serialization", () => {
    it("performs roundtrip serialize -> deserialize", () => {
      ...
      expect(decoded.targetEvmAddress.toLowerCase()).toBe(
        payload.targetEvmAddress.toLowerCase()
      );
    ...
    it("produces stable hex for fixed input", () => {
      ...
      expect(hex1).toBe(
        "4947524100000000000000000000000000000000000000010000000000000001736d00000000"
      );
    });
  });
  ...
  describe("Prefix Mining Simulation", () => {
    ...
    it("fails when max attempts reached", () => {
      ...
      expect(() =>
        simulatePrefixMining(basePayload, impossiblePrefix, { maxAttempts: 10 })
    ...
    it("respects start nonce", () => {
      ...
      const result2 = simulatePrefixMining(basePayload, prefix, {
        initialNonce: result1.nonce + 1
      });
