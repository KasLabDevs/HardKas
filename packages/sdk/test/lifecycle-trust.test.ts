...
describe.skip("0.12.0-rc.16 Lifecycle Integrity & Trust Boundary Tests", () => {
  let sdk: Hardkas;
  ...
  it("1. Fake policyRefs rehashed must fail verification under strict mode", async () => {
    ...
    expect(result.details.some((i: any) => i.code === "REFERENCE_HASH_MISMATCH")).toBe(
      true
    );
  });
  ...
  it("7. Corrupted lineage sequence must fail verification", async () => {
    ...
    (corruptedReceipt as any).contentHash = calculateContentHash(
      corruptedReceipt,
      CURRENT_HASH_VERSION
    );
    ...
    const result = await sdk.artifacts.verify(corruptedReceipt, {
      throwOnInvalid: false,
      strict: true
    });
    expect(result.valid).toBe(false);
    expect(result.details.some((i: any) => i.code === "NON_MONOTONIC_SEQUENCE")).toBe(
      true
    );
  });
