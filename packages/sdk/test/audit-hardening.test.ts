...
describe("Audit Hardening Suite (0.8.16-alpha)", () => {
  let tmpDir: string;
  let sdk: Hardkas;
  ...
  describe("P0.2: Artifact Sandbox Boundary", () => {
    it("should reject path traversal in artifacts.read()", async () => {
      ...
      await expect(sdk.artifacts.read("../secret.json")).rejects.toMatchObject({ code: "PATH_TRAVERSAL" });
    });
    ...
    it("should reject absolute paths outside the workspace", async () => {
      ...
      await expect(sdk.artifacts.read(extFile)).rejects.toMatchObject({ code: "PATH_TRAVERSAL" });
  ...
  describe("P0.3: Replay SDK Path Resolution", () => {
    it("should resolve correct canonical directories for replay", async () => {
      ...
      fs.writeFileSync(path.join(receiptsDir, "tx-receipt-dummy.json"), JSON.stringify({ schema: "hardkas.txReceipt", txId: "plan-dummy-1" }));
      
      const artifactsDir = path.join(sdk.workspace.hardkasDir, "artifacts");
      fs.mkdirSync(artifactsDir, { recursive: true });
      fs.writeFileSync(path.join(artifactsDir, "tx-plan-dummy.json"), JSON.stringify({ schema: "hardkas.txPlan", planId: "plan-dummy-1", createdAt: "2026-06-01T00:00:00Z" }));
  ...
  describe("P1.1: Localnet State Naming Compatibility", () => {
    it("should migrate localnet-state.json to localnet.json", async () => {
      ...
      const localnetStatePath = path.join(sdk.workspace.hardkasDir, "localnet-state.json");
  ...
  describe("P1.2: SDK Mode Resolution", () => {
    it("should resolve to real mode for testnet/mainnet", async () => {
      const realSdk = await Hardkas.open({ cwd: tmpDir, network: "testnet-10", autoBootstrap: false });
