...
describe("SDK Tamper Detection & Forensic Regression", () => {
  ...
  it("should reject tampered signedTx during verify, simulate, and send", async () => {
    ...
    await expect(sdk.tx.simulate(tampered)).rejects.toThrow(/content_hash_mismatch|corrupted or invalid/);
    
    // 3. tx.send must throw
    await expect(sdk.tx.send(tampered as any)).rejects.toThrow(/content_hash_mismatch|corrupted or invalid/);
  });
  ...
  it("should reject tampered plan during verify and sign", async () => {
    ...
    await expect(sdk.tx.sign(tampered as any, 'alice')).rejects.toThrow(/content_hash_mismatch|corrupted or invalid/);
  });
  ...
  it("should reject tampered receipt during replay verify", async () => {
    ...
    const replayResult = await sdk.replay.verify(tampered, { throwOnInvalid: false } as any);
  ...
  it("should pass forensic regression test for forensic-Tamper-Detection-1780477460463.json", async () => {
    ...
    const forensicPath = path.resolve(__dirname, "../../../evidence/crypto-audit-081/forensic-Tamper-Detection-1780477460463.json");
    ...
    const result = await sdk.artifacts.verify(tamperedArtifact, { throwOnInvalid: false });
