...
export const SilverScript = {
  builder() {
    throw new Error(
      "SILVERSCRIPT_MAINNET_NOT_ENABLED: SilverScript builder is experimental and requires simnet capability checks in 0.11.5-alpha."
    );
  }
};
...
export class HardkasSilver {
  ...
  async deploy(
    ...
    if (options.mode === "real") {
      throw new Error(
        "SDK_SILVER_REAL_LIFECYCLE_UNSUPPORTED: use `hardkas silver deploy` for Docker/RPC execution in 0.11.5-alpha."
      );
    }
  ...
  async spend(
    ...
    if (options.mode === "real") {
      throw new Error(
        "SDK_SILVER_REAL_LIFECYCLE_UNSUPPORTED: use `hardkas silver spend` for Docker/RPC execution in 0.11.5-alpha."
      );
    }
