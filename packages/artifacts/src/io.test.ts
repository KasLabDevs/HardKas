...
describe("writeArtifact", () => {
  ...
  it("writes to exact file when a normal file path is provided", async () => {
    ...
    expect(writeFileAtomic).toHaveBeenCalledWith("plan.json", expect.stringContaining("123"));
  });
  ...
  it("auto-generates filename when an existing directory path is provided", async () => {
    ...
    expect(writeFileAtomic).toHaveBeenCalledWith(path.join("somedir", "txPlan-123.json"), expect.stringContaining("123"));
  });
