...
export function loadRealAccountStoreSync(options?: {
  ...
  try {
    ...
    for (const a of store.accounts) {
      if ("privateKey" in a && a.privateKey !== undefined) {
        if (
          a.privateKey === null ||
          a.privateKey === "" ||
          a.privateKey === "[object Object]" ||
          typeof a.privateKey === "object" ||
          (typeof a.privateKey === "string" && a.privateKey.includes("__wbg_ptr")) ||
          (typeof a.privateKey === "object" && "__wbg_ptr" in (a.privateKey as any))
        ) {
          const err = new Error(
          ...
          (err as any).code = "CORRUPTED_PRIVATE_KEY_SERIALIZATION";
          throw err;
        }
      }
    }
