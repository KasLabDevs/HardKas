...
describe("Localnet Property Tests (fast-check)", () => {
      ...
      fc.property(
        ...
        (txCount) => {
          ...
          let state = createInitialLocalnetState({
            ...
            initialBalanceSompi: initialBalance
          });
          ...
          for (let i = 0; i < txCount; i++) {
            ...
            if (result.ok) {
              ...
              const inputSum = plan.inputs.reduce(
                (acc: bigint, input: any) => acc + BigInt(input.amountSompi),
                0n
              );
              const outputSum =
                plan.outputs.reduce(
                  (acc: bigint, output: any) => acc + BigInt(output.amountSompi),
                  0n
          ...
          if (actual !== initialSum) {
            console.error(
              `Mismatch! actual=${actual} (${typeof actual}), initialSum=${initialSum} (${typeof initialSum})`
            );
  ...
  it("should be deterministic: same sequence produces same finalStateHash", () => {
    fc.assert(
      fc.property(fc.integer({ min: 1, max: 5 }), (txCount) => {
        const runSequence = () => {
  ...
  it("should prevent double spending (invariant check)", () => {
    const state = createInitialLocalnetState({
      accounts: 2,
      initialBalanceSompi: 100n * SOMPI_PER_KAS
    });
    ...
    const res1 = applySimulatedPayment(
      state,
      {
        from: "alice",
        to: "bob",
    ...
    const res2 = applySimulatedPayment(
      res1.state,
      {
        from: "alice",
        to: "bob",
