...
describe("HardKAS Lock System", () => {
  const testRoot = path.join(
    os.tmpdir(),
    `hardkas-test-lock-${Math.random().toString(36).slice(2)}`
  );
  ...
  it("should fail to acquire an already held lock", async () => {
    ...
    await expect(acquireLock({ rootDir: testRoot, name: "test-lock" })).rejects.toThrow(
      /Workspace is locked/
    );
