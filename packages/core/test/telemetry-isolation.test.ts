...
describe("Telemetry Isolation", () => {
  it("should keep parallel runs isolated via AsyncLocalStorage", async () => {
    const root1 = path.join(
      os.tmpdir(),
      `hardkas-telemetry-iso-1-${Math.random().toString(36).slice(2)}`
    );
    const root2 = path.join(
      os.tmpdir(),
      `hardkas-telemetry-iso-2-${Math.random().toString(36).slice(2)}`
    );
