  ...
  registerTortureBucket({
    ...
    async run(ctx) {
      ...
      try {
        ...
        fs.writeFileSync(path.join(artifactsDir, `${artIdA}.json`), JSON.stringify({...payloadA, artifactId: artIdA, contentHash: hashA}));
        ...
        fs.writeFileSync(path.join(artifactsDir, `${artIdB.toUpperCase()}.JSON`), JSON.stringify({...payloadB, artifactId: artIdB, contentHash: calculateContentHash(payloadB)}));
        ...
        try {
          ...
          fs.writeFileSync(longPathArt, JSON.stringify({...payloadA, artifactId: `plan-${hashA}`, contentHash: hashA}));
        ...
        if (artifacts.length < 2) {
          throw new TortureInvariantError("Failed to index artifacts with case insensitive extensions", "CASE_INSENSITIVE_FAIL");
        }
  ...
  registerTortureBucket({
    ...
    async run(ctx) {
      ...
      try {
        ...
        fs.writeFileSync(nfcPath, JSON.stringify({...payload, artifactId: artId, contentHash: hash}));

        await backend.sync({ strict: true, cwd: sandboxDir });
        const artifacts = await backend.findArtifacts();

        if (artifacts.length !== 1 || artifacts[0]!.artifactId !== artId) {
           throw new TortureInvariantError("Unicode normalization broke indexing", "UNICODE_FAIL");
        }

        // Test hashing normalization equivalence
        const payloadNFC = createValidMockPayload(ctx, "C:\\test\\café.json".normalize("NFC"));
        const payloadNFD = createValidMockPayload(ctx, "C:\\test\\café.json".normalize("NFD"));
        if (calculateContentHash(payloadNFC) !== calculateContentHash(payloadNFD)) {
           throw new TortureInvariantError("Semantic identity changed under unicode normalization", "UNICODE_IDENTITY_DRIFT");
        }
  ...
  registerTortureBucket({
    ...
    async run(ctx) {
      ...
      try {
        ...
        fs.writeFileSync(file, JSON.stringify({...payload, artifactId: artId, contentHash: hash}));
        ...
        if (artifacts.length !== 1) {
           throw new TortureInvariantError("Future timestamp broke indexing", "FUTURE_MTIME_FAIL");
        }
    ...
    async run(ctx) {
      ...
      try {
        ...
        for(let i=0; i<10; i++) {
            ...
            const hash = calculateContentHash(payload);
            const artId = `plan-${hash.slice(0, 16)}`;
            fs.writeFileSync(path.join(artifactsDir, `${artId}.json`), JSON.stringify({...payload, artifactId: artId, contentHash: hash}));
        }
        ...
        if (artifacts.length !== 10) {
           throw new TortureInvariantError("Filesystem ordering broke sync", "FS_ORDER_FAIL");
        }
  ...
  registerTortureBucket({
    ...
    async run(ctx) {
      ...
      try {
        ...
        if (!caught) {
            throw new TortureInvariantError("Manual truncation was silently ignored", "HUMAN_TRUNCATION_FAIL");
        }
      ...
      try {
        ...
        if (!caught) {
          ...
          if (artifacts.length !== 0) {
              throw new TortureInvariantError("Deleted file by AV left false canonicality", "AV_DELETE_FAIL");
          }
        }
  ...
  registerTortureBucket({
    ...
    async run(ctx) {
      ...
      try {
        ...
        fs.writeFileSync(outsideFile, JSON.stringify({...payload, artifactId: artId, contentHash: hash}));
  ...
  registerTortureBucket({
    ...
    async run(ctx) {
      ...
      try {
        ...
        if (artifacts.length !== 1) {
            throw new TortureInvariantError("CI missing DB state corrupted recovery", "CI_RECOVERY_FAIL");
        }
  ...
  registerTortureBucket({
    ...
    async run(ctx) {
      ...
      try {
        ...
        fs.writeFileSync(tmpFile, JSON.stringify({...payload, artifactId: artId, contentHash: hash}));
