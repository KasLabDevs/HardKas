...
registerTortureBucket({
  ...
  profiles: ["corruption"],
  expectedInvariant: "AppendCoordinator successfully truncates partial JSONL suffix while keeping large JSONL lines during concurrent append simulation",
  run: async (ctx: TortureBucketContext) => {
    ...
    const largePayload = { schema: "hardkas.artifact.v1", type: "large", data: "x".repeat(70000) };
    ...
    fs.appendFileSync(eventsPath, '{"schema":"hardkas.artifact.v1","type":"corrupted","d');
    ...
    if (lines.length !== 2) {
      throw new TortureInvariantError(`Expected exactly 2 lines after recovery, got ${lines.length}`, "CORRUPTION_RECOVERY_FAILED", "catastrophic");
    }
    ...
    try {
      ...
      throw new TortureInvariantError("Recovered JSON lines are invalid", "CORRUPTION_RECOVERY_JSON_PARSE", "catastrophic");
    }

    if (lines[0]!.length < 70000) {
      throw new TortureInvariantError("Large JSONL line was incorrectly truncated", "LARGE_LINE_TRUNCATED", "catastrophic");
    }
