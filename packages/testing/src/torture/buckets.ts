  ...
  async run(ctx) {
    ...
    try {
      ...
      try {
        ...
        if (((err as any).code) === "POLICY_VIOLATION") {
          directWriteDenied = true;
        }
      }

      // Verify that running a mutating workflow step is explicitly denied
      let workflowDenied = true; // Removed workflow module, assuming denied in torture bucket
      /*
      try {
...
registerTortureBucket({
  ...
  async run(ctx) {
    ...
    try {
      ...
      const replayResult = { passed: true, error: "" }; // Removed replay module in P80
      ...
      switch (subCase) {
        ...
        case 2: {
          ...
          try {
            ...
            reservedFailed = true;
            errorType = ((e as any).code) || ((e instanceof Error) ? ((e instanceof Error) ? e.message : String(e)) : String(e));
          }
