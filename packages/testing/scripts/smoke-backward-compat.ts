...
import { generateMigrationReceipt, migrateArtifactPayload, verifyLineage } from "@hardkas/artifacts";
...
async function runSmoke() {
  ...
  for (const file of files) {
    const originalArtifact = JSON.parse(fs.readFileSync(path.join(fixturesDir, file), "utf-8"));
    ...
    try {
      ...
      if (migrated.migrated) {
        const receipt = generateMigrationReceipt(artifact, migrated.artifact, "smoke-test");
        ...
        if (!rLineage.ok || !aLineage.ok) {
          ...
          console.log(`✅ migrationReceipt lineage OK: ${receipt.oldHash} -> ${receipt.contentHash} -> ${receipt.newHash}`);
        }
      } else {
