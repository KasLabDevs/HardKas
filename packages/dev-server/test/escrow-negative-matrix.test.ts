...
// Conditional integration (opt-in): exercises the policy checks over HTTP on a
// real funded escrow. The same checks run unconditionally on seeded records in
// escrow-fail-closed.test.ts.
const INTEGRATION = process.env.HARDKAS_ESCROW_INTEGRATION === "1";

describe.runIf(INTEGRATION)("[conditional integration] Negative Matrix", () => {
