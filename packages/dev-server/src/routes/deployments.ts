...
deploymentsRoutes.get("/", async (c) => {
  ...
  try {
    const deployments = await queryBackend.findArtifacts({ schema: "hardkas.deployment.v1" });
    ...
    })).sort((a, b) => new Date(b.deployedAt || 0).getTime() - new Date(a.deployedAt || 0).getTime());
