...
export function handleError(e, context) {
    ...
    if (e instanceof Error && e.code && e.context) {
        const ctx = e.context;
        getOutput().error(`  ${pc.dim("Code:")}     ${pc.white(e.code)}`);
        if (ctx.endpoint)
...
export function handleLockError(e) {
    ...
    if (code === "LOCK_HELD" || code === "LOCK_TIMEOUT" || code === "STALE_LOCK") {
        ...
        if (meta) {
            ...
            if (meta.path)
                getOutput().error(`  ${pc.dim("Path:")}    ${pc.white(meta.path)}`);
        }
        getOutput().error(pc.cyan(`\n  💡 Suggestion:`));
        if (code === "STALE_LOCK") {
            getOutput().error(`    The process (PID ${meta?.pid}) appears to be dead.`);
            getOutput().error(`    Run 'hardkas lock clear ${meta?.name} --if-dead' to release it safely.`);
        }
        else {
