...
export async function runCorpusVerify(options) {
    ...
    if (opManifest) {
        expectEqual(opManifest.schema, HardkasSchemas.ToccataGoldenManifestV1, issues, "OP_TRUE_SCHEMA_INVALID", opManifestPath);
        validateCommonClaims(opManifest, issues, opManifestPath);
    }
    if (failureManifest) {
        expectEqual(failureManifest.schema, HardkasSchemas.ToccataGoldenFailureManifestV1, issues, "FAILURE_SCHEMA_INVALID", failureManifestPath);
    ...
    if (opManifest?.files) {
        for (const file of opManifest.files) {
            ...
            if (file === "manifest.json" || file === "compare-report.json")
                continue;
    ...
    if (compareReport) {
        expectEqual(compareReport.schema, HardkasSchemas.ToccataGoldenCompareV1, issues, "COMPARE_SCHEMA_INVALID", comparePath);
    ...
    if (failureManifest?.cases) {
        for (const entry of failureManifest.cases) {
            ...
            if (!fixture)
                continue;
            expectEqual(fixture.schema, HardkasSchemas.ToccataGoldenFailureCaseV1, issues, "FAILURE_CASE_SCHEMA_INVALID", filePath);
            ...
            const expectedError = fixture.expectedSimulator?.error ||
...
//# sourceMappingURL=corpus-verify-runner.js.map