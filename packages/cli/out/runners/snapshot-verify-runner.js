...
export async function runSnapshotVerify(options) {
    try {
        ...
        const snapshot = state.snapshots?.find((s) => s.id === options.idOrName ||
        ...
        if (result.ok) {
            ...
            console.log(`  Accounts Hash:  âœ“ MATCH (${snapshot.accountsHash?.slice(0, 8)}...)`);
            console.log(`  UTXO Set Hash:  âœ“ MATCH (${snapshot.utxoSetHash?.slice(0, 8)}...)`);
            console.log(`  State Hash:     âœ“ MATCH (${snapshot.stateHash?.slice(0, 8)}...)`);
            console.log(`  Content Hash:   âœ“ MATCH (${snapshot.contentHash?.slice(0, 8)}...)`);
        }
        else {
        ...
        if ((e.name) === "HardkasCliError")
            throw e;
        const { HardkasCliError } = await import("../cli-errors.js");
        throw new HardkasCliError("VERIFICATION_FAILED", `Verification failed: ${((e instanceof Error) ? ((e instanceof Error) ? e.message : String(e)) : String(e))}`, { exitCode: 1 });
    }
}
//# sourceMappingURL=snapshot-verify-runner.js.map