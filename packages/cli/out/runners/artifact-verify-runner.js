...
export async function runArtifactVerify(options) {
    ...
    if (isDir) {
        if (options.recursive) {
            ...
            const { HardkasCliError } = await import("../cli-errors.js");
            throw new HardkasCliError("IS_DIRECTORY", `${options.path} is a directory. Use --recursive to verify all artifacts within it.`, { exitCode: 1 });
        }
    }
...
async function runRecursiveVerify(dir, options) {
    ...
    for (const file of files) {
        ...
        if (result.ok) {
            if (!options.json)
                console.log(`  âœ“ ${relativePath.padEnd(40)} [MATCH]`);
            successCount++;
        }
        else {
            if (!options.json) {
...
//# sourceMappingURL=artifact-verify-runner.js.map