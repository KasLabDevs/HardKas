...
export async function runTxBatch(options) {
    ...
    try {
        ...
        if (Array.isArray(parsed)) {
            ...
            throw new Error("Invalid batch file format. Expected:\n" +
                '  Format 1 (array): [{"from":"alice","to":"bob","amount":"10"}]\n' +
                '  Format 2 (object): {"payments":[{"from":"alice","to":"bob","amount":"10"}]}');
        }
    }
    catch (e) {
        if (e instanceof Error && ((e instanceof Error) ? ((e instanceof Error) ? e.message : String(e)) : String(e)).startsWith("Invalid batch file format"))
            throw e;
        throw new Error(`Failed to read or parse batch file ${file}: ${e instanceof Error ? ((e instanceof Error) ? ((e instanceof Error) ? e.message : String(e)) : String(e)) : String(e)}`);
    }
    const results = [];
    let successCount = 0;
    let failCount = 0;
    if (!json) {
        getOutput().writeLine(`Processing batch of ${payments.length} transactions sequentially...`);
    }
    ...
    for (let i = 0; i < payments.length; i++) {
        ...
        if (!payment.from || !payment.to || !payment.amount) {
            if (!json)
                getOutput().error(`[${i + 1}/${payments.length}] Skipping invalid payment object (missing from/to/amount).`);
            results.push({
                index: i,
                ok: false,
        ...
        try {
            if (!json) {
                getOutput().writeLine(`[${i + 1}/${payments.length}] Flow: ${payment.from} -> ${payment.to} (${payment.amount} KAS) on ${network}`);
            }
            ...
            if (flowResult.ok) {
                successCount++;
                if (!json)
                    getOutput().writeLine(`  ✓ Success`);
            }
            else {
                failCount++;
                if (!json)
                    getOutput().writeLine(`  ✗ Failed: Flow did not complete ok. (plan: ${flowResult.steps.plan.error}, sign: ${flowResult.steps.sign.error}, send: ${flowResult.steps.send.error})`);
            }
        }
        catch (e) {
            failCount++;
            ...
            if (!json)
                getOutput().error(`  ✗ Error: ${errorMsg}`);
        }
    }
    if (json) {
        UI.writeJson({
...
//# sourceMappingURL=tx-batch-runner.js.map