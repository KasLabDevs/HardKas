...
export async function runTxProfile(options) {
    ...
    getOutput().writeLine(`  Amount:     ${formatSompiToKas(BigInt(plan.amountSompi))} (${plan.amountSompi} sompi)`);
    getOutput().writeLine(`  Total Mass: ${result.mass}`);
    getOutput().writeLine(`  Est. Fee:   ${formatSompiToKas(BigInt(plan.estimatedFeeSompi))} (${plan.estimatedFeeSompi} sompi)`);
    getOutput().writeLine("\nMass Breakdown:");
    getOutput().writeLine(`  Base Transaction:  ${result.breakdown.base.toString().padStart(5)}`);
    getOutput().writeLine(`  Inputs (${plan.inputs.length}):       ${result.breakdown.inputs.toString().padStart(5)}`);
    getOutput().writeLine(`  Outputs (${plan.outputs.length + (plan.change ? 1 : 0)}):      ${result.breakdown.outputs.toString().padStart(5)}`);
    if (result.breakdown.payload > 0n) {
        getOutput().writeLine(`  Payload:           ${result.breakdown.payload.toString().padStart(5)}`);
    }
    getOutput().writeLine(`  -----------------------`);
    ...
    plan.inputs.forEach((i, idx) => {
        getOutput().writeLine(`    [${idx}] ${i.outpoint.transactionId.substring(0, 8)}...:${i.outpoint.index} (${formatSompiToKas(BigInt(i.amountSompi))})`);
    });
    getOutput().writeLine(`  Outputs: ${plan.outputs.length + (plan.change ? 1 : 0)}`);
    plan.outputs.forEach((o, idx) => {
        getOutput().writeLine(`    [${idx}] ${o.address.substring(0, 20)}... (${formatSompiToKas(BigInt(o.amountSompi))})`);
    });
    if (plan.change) {
        getOutput().writeLine(`    [C] ${plan.change.address.substring(0, 20)}... (${formatSompiToKas(BigInt(plan.change.amountSompi))}) [CHANGE]`);
    }
    getOutput().writeLine("\nNote: Mass estimation is protocol-aware (0.12.0-rc.19 best-effort).");
}
