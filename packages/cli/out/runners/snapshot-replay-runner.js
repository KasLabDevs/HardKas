...
export async function runSnapshotReplay(options) {
    try {
        ...
        if (!options.json)
            console.log(pc.yellow("  Restoring artifacts to workspace..."));
        try {
        ...
        if (!options.json)
            console.log(pc.yellow("  Rebuilding state projections..."));
        ...
        const indexer = new HardkasIndexer(store.getDatabase(), options.workspaceRoot
        ...
        if (!options.json) {
            UI.causality(`Snapshot Replay: ${options.name}`, {
        ...
        throw new HardkasCliError("SNAPSHOT_REPLAY_FAILED", `Snapshot replay failed: ${((err instanceof Error) ? ((err instanceof Error) ? err.message : String(err)) : String(err))}`, { exitCode: 1, cause: err });
    }
}
//# sourceMappingURL=snapshot-replay-runner.js.map