...
export async function runDevCreate(name) {
    ...
    if (fs.existsSync(targetDir)) {
        if (fs.readdirSync(targetDir).length > 0) {
            ...
            throw new HardkasCliError("DIR_NOT_EMPTY", `Directory '${name}' already exists and is not empty.`, {
                exitCode: 1,
                suggestion: "To initialize an existing project, run 'hardkas dev init' inside the directory."
            });
        }
    }
    else {
    ...
    try {
        ...
        const { HardkasCliError } = await import("../cli-errors.js");
        throw new HardkasCliError("TEMPLATE_GENERATION_FAILED", `Template generation failed: ${((e instanceof Error) ? ((e instanceof Error) ? e.message : String(e)) : String(e))}`, { exitCode: 1 });
    }
}
