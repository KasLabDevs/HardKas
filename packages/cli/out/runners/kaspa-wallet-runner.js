...
export async function runKaspaWalletCreate(name, options) {
    try {
        const { createLocalKaspaWallet } = await import("@hardkas/accounts");
        console.log(pc.bold("\nâ”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”"));
        console.log(pc.bold(`HardKAS â€¢ Kaspa Wallet Creation`));
        console.log(pc.bold("â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”\n"));
...
export async function runKaspaWalletList(options) {
    try {
        const config = await loadHardkasConfig();
        const { listHardkasAccounts } = await import("@hardkas/accounts");
        const accounts = listHardkasAccounts(config.config).filter((a) => a.kind === "kaspa" || a.kind === "synthetic");
        if (options.json) {
...
export async function runKaspaWalletAddress(name) {
    try {
        const config = await loadHardkasConfig();
...
export async function runKaspaWalletBalance(name, options) {
    try {
        const config = await loadHardkasConfig();
        const { resolveHardkasAccountAddress } = await import("@hardkas/accounts");
...
export async function runKaspaWalletSend(from, to, options) {
    try {
        const config = await loadHardkasConfig();
        ...
        const networkId = typeof configObj.networkId === "string"
            ? configObj.networkId
        ...
        const planArtifact = {
            ...
            ...(plan.change
                ...
                : {}),
            contentHash: "synthetic-plan-hash"
        };
        ...
        const submitResult = await client.submitTransaction(JSON.parse(signedArtifact.signedTransaction.payload));
        if (submitResult.accepted) {
            ...
            const { HardkasCliError } = await import("../cli-errors.js");
            throw new HardkasCliError("TX_REJECTED", `Transaction rejected by node.\nDetails: ${JSON.stringify(submitResult.raw)}`, { exitCode: 1 });
        }
    }
    catch (e) {
        if ((e.name) === "HardkasCliError")
...
//# sourceMappingURL=kaspa-wallet-runner.js.map