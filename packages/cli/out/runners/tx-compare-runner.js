...
export async function runTxCompare(input) {
    ...
    for (const item of semanticMatches) {
        if (item.a === item.b) {
            ...
            getOutput().writeLine(`  ${pc.red("âœ—")} ${item.name.padEnd(25)} : SIM: ${item.a}  |  REAL: ${item.b}`);
            matchesFail = true;
        }
    }
    getOutput().writeLine("");
    ...
    for (const item of deltas) {
        ...
        getOutput().writeLine(`  ${color("Î”")} ${item.name.padEnd(25)} : SIM: ${item.a || "N/A"}  |  REAL: ${item.b || "N/A"}`);
    }
    getOutput().writeLine("");
    if (matchesFail) {
