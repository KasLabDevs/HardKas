...
export async function runTest(options) {
    ...
    // 2. File discovery
    const searchPatterns = files.length > 0 ? files : ["test/**/*.test.ts", "tests/**/*.test.ts", "scenarios/**/*.scenario.ts", "scenarios/**/*.test.ts"];
    if (!json) {
    ...
    try {
        ...
        if (options.evidence && scenarioResults.length > 0) {
            if (!json)
                UI.info(`Auto-packaging evidence for ${scenarioResults.length} scenarios...`);
            ...
            for (const resPath of scenarioResults) {
                try {
                    ...
                    if (!json)
                        UI.success(`Packed evidence: ${path.relative(workspaceRoot, outPath)}`);
                }
                catch (err) {
                    if (!json)
                        UI.error(`Failed to pack evidence for ${resPath}: ${err.message}`);
                }
            }
        }
        if (json) {
        ...
        if (!passed) {
            ...
            throw new HardkasCliError("TESTS_FAILED", "One or more scenarios failed.", { exitCode: 1 });
        }
    }
    catch (e) {
        if (e instanceof Error) {
            ...
            if (errCode === "ERR_MODULE_NOT_FOUND" || ((e instanceof Error) ? ((e instanceof Error) ? e.message : String(e)) : String(e)).includes("vitest")) {
                ...
                UI.divider();
                const { HardkasCliError } = await import("../cli-errors.js");
                throw new HardkasCliError("VITEST_MISSING", "Fallback: No real tests were executed because the engine is missing.", { exitCode: 1 });
            }
        }
        throw e;
    }
