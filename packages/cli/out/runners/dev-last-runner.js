...
export async function runDevLast(options) {
    ...
    const latestReceipt = files.find((f) => f.schema.startsWith(HardkasSchemas.TxReceipt) || f.name.startsWith("receipt_"));
    const latestSigned = files.find((f) => f.schema.startsWith(HardkasSchemas.SignedTx) || f.name.includes(".signed"));
    const latestPlan = files.find((f) => f.schema.startsWith(HardkasSchemas.TxPlan) ||
    ...
    if (options.replay) {
        if (target.schema.startsWith(HardkasSchemas.TxReceipt) ||
...
//# sourceMappingURL=dev-last-runner.js.map