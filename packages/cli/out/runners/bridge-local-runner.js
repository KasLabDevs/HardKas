...
export async function runBridgeLocalPlan(options) {
    try {
        ...
        const artifactData = {
            ...
            plan: JSON.parse(JSON.stringify(plan, (k, v) => (typeof v === "bigint" ? v.toString() : v))),
        ...
        console.log(pc.dim("----------------------------------------"));
        console.log(`Fee (est):       ${pc.yellow(formatSompiToKas(plan.estimatedFeeSompi))} KAS`);
...
export async function runBridgeLocalSimulate(options) {
    try {
        ...
        if (!fs.existsSync(artifactsDir))
            fs.mkdirSync(artifactsDir, { recursive: true });
        const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
        const artifactPath = path.join(artifactsDir, `${timestamp}-bridge-local-simulate.json`);
        const artifactData = {
            schema: HardkasSchemas.BridgeLocalSimulationV1,
            status: "success",
            session: { source: ctx.source, name: ctx.sessionName || null },
...
export async function runBridgeLocalInspect(txid, options) {
    try {
...
//# sourceMappingURL=bridge-local-runner.js.map