...
// Telemetry Inspect
// ---------------------------------------------------------------------------
export async function runTelemetryInspect(options) {
    const p = telemetryPath();
    console.log(pc.bold("\nâ”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”"));
    console.log(pc.bold(`HardKAS â€¢ Telemetry Source Inspector`));
    console.log(pc.bold("â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”\n"));
    if (!fs.existsSync(p)) {
    ...
    for (const ev of recent) {
        ...
        const sevColor = ev.severity === "critical"
            ? pc.red
...
export async function runTelemetryVerify() {
    ...
    if (!fs.existsSync(p)) {
        ...
        throw new HardkasCliError("TELEMETRY_FILE_NOT_FOUND", `Verification FAILED: Telemetry file does not exist at ${p}`, { exitCode: 1 });
    }
    const raw = fs.readFileSync(p, "utf-8").trim();
    if (!raw) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        try {
            ...
            console.log(`${pc.red("âœ— Line " + (i + 1) + ":")} Invalid JSON structure (${((e instanceof Error) ? ((e instanceof Error) ? e.message : String(e)) : String(e))})`);
        ...
        if (event.schemaVersion !== HardkasSchemas.TelemetryV1) {
            errors.push(`Invalid schemaVersion (Expected HardkasSchemas.TelemetryV1, got "${event.schemaVersion}")`);
        }
        if (!event.eventId)
    ...
    if (invalidCount > 0) {
        const { HardkasCliError } = await import("../cli-errors.js");
        throw new HardkasCliError("TELEMETRY_VERIFICATION_FAILED", `Telemetry verification FAILED with ${invalidCount} schema violations.`, { exitCode: 1 });
    }
    else {
...
// ---------------------------------------------------------------------------
export async function runTelemetryTail(options) {
    const p = telemetryPath();
    if (!fs.existsSync(p)) {
        const { HardkasCliError } = await import("../cli-errors.js");
        throw new HardkasCliError("TELEMETRY_FILE_NOT_FOUND", `Telemetry file does not exist at ${p}`, { exitCode: 1 });
    }
    const printTail = (linesCount) => {
...
//# sourceMappingURL=telemetry-runners.js.map