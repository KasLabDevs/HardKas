import { loadOrCreateLocalnetState, saveLocalnetState, createSimulatedDag, moveSink } from "@hardkas/localnet";
 ...
 */
export async function runDagSimulateReorg(options) {
    const state = await loadOrCreateLocalnetState();
    if (!state.dag) {
    ...
    // Minimal 0.12.0-rc.12 implementation:
    ...
    if (!forkPointId)
        throw new Error("Could not find fork point in current path.");
    const forkPoint = state.dag.blocks[forkPointId];
    if (!forkPoint)
        throw new Error(`Fork point block ${forkPointId} not found in state.`);
    ...
    const txProvider = (_id) => undefined;
...
//# sourceMappingURL=dag-runners.js.map