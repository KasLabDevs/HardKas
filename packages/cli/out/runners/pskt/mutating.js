...
export async function runPsktExport(options) {
    if (options.json)
        UI.setJsonMode(true);
...
export async function runPsktImport(options) {
...
export async function runPsktSign(sessionPath, options) {
    if (options.json)
        UI.setJsonMode(true);
    let privateKey;
...
export async function runPsktMerge(sessionA, sessionB, options) {
    if (options.json)
        UI.setJsonMode(true);
...
export async function runPsktFinalize(sessionPath, options) {
    if (options.json)
        UI.setJsonMode(true);
    const session = await loadSession(sessionPath);
...
export async function runPsktExtract(sessionPath, options) {
    if (options.json)
        UI.setJsonMode(true);
    const session = await loadSession(sessionPath);
    ...
    if (!options.force) {
        try {
            ...
            if (e.code !== "ENOENT" && !(e instanceof HardkasCliError))
                throw e;
            if (e instanceof HardkasCliError)
                throw e;
        }
    }
