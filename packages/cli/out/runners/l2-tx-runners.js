...
import { resolveL2Profile, EvmJsonRpcClient, toHexQuantity, normalizeEvmTransactionReceipt } from "@hardkas/l2";
import { loadHardkasConfig } from "@hardkas/config";
import { assertValidIgraTxPlanArtifact, assertValidIgraSignedTxArtifact, writeArtifact, readArtifact, HARDKAS_VERSION, ARTIFACT_SCHEMAS, createIgraPlanId, createIgraSignedId, assertValidIgraTxReceiptArtifact, listIgraTxReceiptArtifacts, loadIgraTxReceiptArtifact, calculateContentHash } from "@hardkas/artifacts";
import { loadRealAccountStore, resolveRealAccountOrAddress } from "@hardkas/accounts";
export async function runL2TxBuild(options) {
    ...
    if (options.data)
        assertHexData(options.data, "data");
...
export async function runL2TxSign(options) {
    ...
    if (plan.request.from)
        console.log(`From:      ${plan.request.from}`);
...
export async function runL2TxSend(options) {
    const loaded = await loadHardkasConfig();
    ...
    const isMainnet = profile.name === "mainnet" ||
        profile.name.includes("mainnet") ||
        artifact.networkId === "mainnet" ||
        artifact.chainId === 1 ||
        profile.chainId === 1;
    if (isMainnet) {
        throw new Error("L2 mainnet broadcast is disabled in HardKAS 0.12.0-rc.18.");
    }
    if (!options.yes) {
        const { HardkasCliError } = await import("../cli-errors.js");
        throw new HardkasCliError("CANCELED", "L2 broadcast cancelled: --yes flag is required for this phase.", { exitCode: 1 });
    }
    ...
    if (String(remoteChainId) !== String(artifact.chainId)) {
        ...
        console.log("");
        const { HardkasCliError } = await import("../cli-errors.js");
        throw new HardkasCliError("CHAIN_ID_MISMATCH", "Artifact chain ID does not match remote RPC chain ID.", { exitCode: 1 });
    }
    // 4. Send
...
export async function runL2TxReceipt(options) {
    const loaded = await loadHardkasConfig();
    let localReceipt;
    try {
...
export async function runL2TxReceipts(options) {
...
export async function runL2TxStatus(options) {
    const loaded = await loadHardkasConfig();
    const profile = resolveL2Profile({
        name: options.network,
        userProfiles: loaded.config.l2?.networks,
...
//# sourceMappingURL=l2-tx-runners.js.map