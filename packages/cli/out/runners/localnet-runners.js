...
export async function runLocalnetStart(opts) {
...
export async function runLocalnetStop(opts) {
...
export async function runLocalnetStatus(opts) {
    ...
    if (node.serverVersion)
        UI.info(`Version: ${node.serverVersion}`);
    if (node.virtualDaaScore)
        UI.info(`DAA: ${node.virtualDaaScore}`);
}
export async function runLocalnetFund(opts) {
    const profile = opts.profile || TOCCATA_PROFILE;
    if (profile !== TOCCATA_PROFILE) {
    ...
    const status = current.matureBalanceSompi > before.matureBalanceSompi
...
async function getVirtualFingerprint(rpc) {
...
function computeUtxoSetHash(utxos) {
    const keys = utxos
        .map(u => [
...
async function waitForVirtualDagSettle(rpc, opts) {
...
async function waitForFundingSpendability(rpc, opts) {
    const REQUIRED = opts.stablePolls ?? 3;
...
export async function runLocalnetFork(opts) {
    ...
    if (!rpcUrl)
        throw new Error(`No RPC URL configured for network '${opts.network}'.`);
...
async function detectToccataNode(quiet = false) {
    ...
    try {
        ...
        return {
            ...
            networkId: serverNetworkId === "unknown"
                ? "simnet"
...
async function restartToccataMiner(address) {
...
async function getAddressFundingState(address, quiet = false) {
...
async function withOptionalSilentConsole(quiet, fn) {
    if (!quiet)
        return fn();
...
//# sourceMappingURL=localnet-runners.js.map