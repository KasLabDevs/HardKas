...
export async function runSemanticVerify(options) {
    if (!options.ciMode) {
        ...
        throw new HardkasCliError("CI_MODE_REQUIRED", "verify-semantics currently only supports --ci-mode for cross-platform validation.", { exitCode: 1 });
    }
    const reportsDir = path.join(process.cwd(), ".hardkas", "reports");
    if (!fs.existsSync(reportsDir)) {
        const { HardkasCliError } = await import("../cli-errors.js");
        throw new HardkasCliError("NO_REPORTS_FOUND", `No torture reports found in ${reportsDir}. Run torture matrix first.`, { exitCode: 1 });
    }
