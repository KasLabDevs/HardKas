import { parseKasToSompi, systemRuntimeContext } from "@hardkas/core";
 ...
 */
export async function runTxPlan(input) {
    const { targetName, from, to, amount, networkId, feeRate, config, url, workspaceRoot, workflowId, assumptionLevel } = input;
    ...
    if (isHardkasSimulatedAccount && backend !== "simulator") {
        throw new Error("NETWORK_ACCOUNT_MISMATCH: Cannot use a simulated account with a real network or RPC provider.");
    }
    let availableUtxos = [];
    let mode = "simulator";
    let plan = null;
    let rpcUrl = providerConfig.endpoint;
    const planCoinbaseMaturity = getCoinbaseMaturity(resolvedNetwork, configNetworkKind === "kaspa-node" || configNetworkKind === "kaspa-rpc" || configNetworkKind === "simulated"
        ? networkDef.consensusParams
        : undefined);
    let stateAddress;
    if (backend === "simulator") {
        const { loadOrCreateLocalnetState, getSpendableUtxos, resolveAccountAddressFromState } = await import("@hardkas/localnet");
        ...
        try {
            ...
            const getVirtualFingerprint = async (rpc) => {
            ...
            for (let attempt = 0; attempt < MAX_PLAN_RETRIES; attempt++) {
                ...
                if (actualFeeRate === undefined) {
                    ...
                    const tempFees = new HardkasFees({ provider: { rpcUrl: rpcUrl }, config: { cwd: workspaceRoot || process.cwd(), config: resolvedConfig } });
                ...
                const confirmSet = new Set(confirmUtxos.map(u => `${u.outpoint.transactionId}:${u.outpoint.index}`));
                const allPresent = candidatePlan.inputs.every(inp => confirmSet.has(`${inp.outpoint.transactionId}:${inp.outpoint.index}`));
                if (!allPresent) {
                    continue; // retry READ phase
                }
                plan = candidatePlan;
            ...
            if (e instanceof UtxoVirtualStateUnstableError)
                throw e;
            if (e instanceof Error && e.message.includes("No UTXOs found"))
                throw e;
            ...
            if (errCode === "RPC_SCHEMA_ERROR") {
                throw new RpcSchemaError({
                    ...
                    suspectedCause: "This endpoint might be running a node version that uses a different response schema for UTXOs.",
...
//# sourceMappingURL=tx-plan-runner.js.map