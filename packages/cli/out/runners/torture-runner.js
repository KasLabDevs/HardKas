...
import { getAllTortureBuckets, LcgPrng } from "@hardkas/testing";
import { EnvironmentTelemetry, AppendCoordinator } from "@hardkas/core";
import { HardkasSchemas } from "@hardkas/artifacts";
/**
 * Deterministically generates the sequence of buckets and caseSeeds from the global seed.
 */
function getTestCaseSchedule(globalSeed, totalIterations, profileFilter) {
...
export async function runTortureMatrix(options) {
    // 1. Resolve Seed
    let seed = 0;
    ...
    UI.info(`  ${pc.dim("Active Buckets:")} ${pc.green(getAllTortureBuckets()
        .map((b) => b.name)
        .join(", "))}\n`);
    ...
    for (const item of schedule) {
        ...
        let filesystemMode;
        let symlinkMode;
        let normalizedPathStrategy;
        let clockSkewDetected;
        let externalMutationDetected;
        let longPathSupportDetected;
        let sandboxSnapshotPath;
        ...
        const eventSeverity = status === "FAILED_CASE"
            ? "critical"
            : item.iteration % 10 === 0
                ? "elevated"
                : "nominal";
        ...
        UI.info(`  ${indicator} [${pc.cyan(item.caseId)}] [${pc.blue(bucket.name.padEnd(28))}] -> ${statusText} ${pc.dim(`(${duration}ms)`)}`);
    ...
    if (fs.existsSync(telemetryPath)) {
        try {
            ...
            const bucketAnomalies = {};
            const typeAnomalies = {};
            for (const ev of events) {
                if (ev.bucket) {
                    bucketAnomalies[ev.bucket] = (bucketAnomalies[ev.bucket] || 0) + 1;
                }
                const evType = ev.type || ev.anomalyType || "UNKNOWN";
...
export async function runTortureReplay(options) {
    ...
    try {
        ...
        throw new HardkasCliError("TORTURE_REPLAY_FAILED", "Case replay invariant violated.", { exitCode: 1 });
    }
}
//# sourceMappingURL=torture-runner.js.map