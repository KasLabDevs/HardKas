...
export async function runAccountsConsolidate(options) {
    ...
    if (resolvedName === "mainnet" && options.execute && !options.allowMainnet) {
        ...
        err.code = "MAINNET_CONSOLIDATION_BLOCKED";
        throw err;
    }
    ...
    if (provider.mode !== "simulator") {
        ...
        sdk.rpc = new JsonWrpcKaspaClient({ rpcUrl: provider.endpoint });
    }
    ...
    try {
        ...
        if (((e instanceof Error) ? ((e instanceof Error) ? e.message : String(e)) : String(e)) && ((e instanceof Error) ? ((e instanceof Error) ? e.message : String(e)) : String(e)).includes("connection error")) {
            ...
            err.code = "RPC_CONNECTION_FAILED";
            throw err;
        }
    ...
    if (beforeCount <= options.targetUtxos) {
        ...
        err.code = "CONSOLIDATION_NOT_REQUIRED";
        throw err;
    }
    ...
    // Sort smallest-first
    allUtxos.sort((a, b) => {
        if (a.amountSompi < b.amountSompi)
            return -1;
        if (a.amountSompi > b.amountSompi)
            return 1;
        return 0;
    ...
    while (remaining > options.targetUtxos && idx < allUtxos.length) {
        ...
        if (size < 2)
            break; // Cannot consolidate 1 input
    ...
    if (options.dryRun || !options.execute) {
        if (options.json) {
            getOutput().writeLine(JSON.stringify({
            ...
            for (let i = 0; i < batches.length; i++) {
                getOutput().writeLine(` Batch ${i + 1}: ${batches[i].length} inputs -> 1 output`);
            }
    ...
    for (let i = 0; i < batches.length; i++) {
        ...
        if (!options.json) {
            getOutput().writeLine(`Submitted batch ${i + 1}/${batches.length}: ${receipt.txId}`);
        }
    }
    ...
    if (options.json) {
        ...
        getOutput().writeLine(`Check final balance with: hardkas accounts balance ${resolvedAccount.name}`);
    }
}
//# sourceMappingURL=accounts-consolidate-runner.js.map