...
export function registerTaskCommands(program, loadedConfig) {
    ...
    if (taskNames.length === 0) {
        ...
        taskGroup
            ...
            .allowUnknownOption()
            .action(async (name) => {
            throw new HardkasCliError("TASK_NOT_FOUND", `Task '${name}' not found. No tasks are defined in hardkas.config.ts.`, { exitCode: 1 });
        });
        return;
    }
    ...
    taskGroup.on("command:*", (operands) => {
        throw new HardkasCliError("TASK_NOT_FOUND", `Task '${operands[0]}' not found. Run 'hardkas task --help' to see available tasks.`, { exitCode: 1 });
    });
}
