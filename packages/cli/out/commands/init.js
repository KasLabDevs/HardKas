...
`;
                    ...
                    if (!options.json)
                        UI.info("Created: vitest.config.ts");
                }
                const template = `import { defineHardkasConfig } from "@hardkas/sdk";

export default defineHardkasConfig({
  // HardKAS v0.12.0-rc.18 Configuration
  execution: {
    default: "simulator",
    targets: {
      simulator: {
...
`;
                if (!fs.existsSync(testFile)) {
                    ...
                    if (!options.json)
                        UI.info("Created: test/payment.scenario.ts");
                }
                // Hardened .gitignore
                ...
                const isSimulatedDefault = !options.network || options.network === "simulated";
                if (isSimulatedDefault) {
                    try {
                        ...
                        if (!options.json)
                            UI.info("Created: .hardkas/localnet.json (simulated accounts funded: 1000 KAS each)");
                    }
                    catch {
                ...
                if (options.json) {
                    ...
                    UI.info(`Created: hardkas.config.ts (0.12.0-rc.18)`);
