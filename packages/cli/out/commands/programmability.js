...
export function registerProgrammabilityCommands(program) {
    ...
    app
        ...
        .action(async (options) => {
        const sdk = await createSdk();
        const kind = normalizeKind(options.kind, true);
...
function normalizeInspectKind(kind) {
    if (kind === "silver" || kind === "zk" || kind === "vprog")
        return kind;
    throw new Error(`PROGRAMMABILITY_KIND_INVALID: ${kind}`);
}
function normalizeKind(kind, allowFullLab) {
    if (kind === "silver" || kind === "zk" || kind === "vprog")
        return kind;
    if (allowFullLab && kind === "full-lab")
        return kind;
    throw new Error(`PROGRAMMABILITY_KIND_INVALID: ${kind}`);
}
