...
function renderHumanReadable(caps) {
    getOutput().writeLine(`${pc.bold("HardKAS")} ${pc.cyan("v" + caps.version)} — ${pc.green("Hardened Alpha")}\n`);
    ...
    getOutput().writeLine(`    Replay:      ${pc.dim(caps.trustBoundaries.replay.replace(/-/g, " "))}`);
    getOutput().writeLine(`    Artifacts:   ${pc.dim(caps.trustBoundaries.artifacts.replace(/-/g, " "))}`);
    getOutput().writeLine(`    Simulator:   ${pc.dim(caps.trustBoundaries.simulator.replace(/-/g, " "))}`);
    getOutput().writeLine(`    Query store: ${pc.dim(caps.trustBoundaries.queryStore.replace(/-/g, " "))}`);
    getOutput().writeLine(`    L2 bridge:   ${pc.dim(caps.trustBoundaries.l2Bridge.replace(/-/g, " "))}`);
}
