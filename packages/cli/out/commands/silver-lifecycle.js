...
export function getSilverDeployPlanCommand() {
    return new Command("deploy-plan")
        ...
        if (opts.network !== "simnet") {
            getOutput().error(pc.red(`SILVERSCRIPT_MAINNET_NOT_ENABLED: Only simnet is supported for SilverScript lifecycle.`));
            throw new Error("Command failed");
        }
        if (!fs.existsSync(compileArtifactPath)) {
            getOutput().error(pc.red(`Error: Compile artifact not found at ${compileArtifactPath}`));
            throw new Error("Command failed");
        }
        ...
        deployPlan.contentHash = contentHash;
        deployPlan.artifactId = `silverdeployplan-${contentHash.substring(0, 16)}`;
        const outPath = path.resolve(process.cwd(), `${deployPlan.artifactId}.json`);
        await writeArtifact(outPath, deployPlan);
...
export function getSilverDeployCommand() {
    return new Command("deploy")
        ...
        if (deployPlan.networkId !== "simnet") {
            getOutput().error(pc.red(`SILVERSCRIPT_MAINNET_NOT_ENABLED: Only simnet is supported.`));
            throw new Error("Command failed");
        }
        const { Hardkas } = await import("@hardkas/sdk");
        const sdk = await Hardkas.create({
        ...
        if (!pkValue) {
            getOutput().error(pc.red(`Error: Please provide --private-key <hex> to sign the deployment funding.`));
            throw new Error("Command failed");
        }
        ...
        try {
            ...
            if (!rpcUrl)
                throw new Error("Could not resolve RPC URL");
            const client = new JsonWrpcKaspaClient({ rpcUrl });
            const info = await client.getInfo();
            utxos = (await client.getUtxosByAddress(address.toString())).filter((u) => {
                if (!u.isCoinbase)
                    return true;
        ...
        if (totalInput < targetSompi + feeSompi) {
            getOutput().error(pc.red(`Insufficient funds. Need ${targetSompi + feeSompi}, got ${totalInput}`));
            throw new Error("Command failed");
        }
        ...
        const txSignable = kaspa.createTransaction(kaspaUtxos, outputs, changeAddress, feeSompi, undefined, 1, 1);
        txSignable.tx.outputs[0].scriptPublicKey = new kaspa.ScriptPublicKey(deployPlan.scriptPublicKeyVersion, deployPlan.lockingScriptHex);
        let signedTx;
        try {
        ...
        deployArtifact.contentHash = contentHash;
        deployArtifact.artifactId = `silverdeploy-${contentHash.substring(0, 16)}`;
        const outPath = path.resolve(process.cwd(), `${deployArtifact.artifactId}.json`);
        await writeArtifact(outPath, deployArtifact);
        await sdk.rpc.close();
...
export function getSilverSpendPlanCommand() {
    return new Command("spend-plan")
        ...
        if (!fs.existsSync(deployArtifactPath)) {
            getOutput().error(pc.red(`Error: Deploy artifact not found at ${deployArtifactPath}`));
            throw new Error("Command failed");
        }
        ...
        if (lock.lockingScriptHex !== deployArtifact.lockingScriptHex) {
            getOutput().error(pc.red(`SILVERSCRIPT_LOCKING_SCRIPT_MISMATCH: Computed locking script does not match deploy artifact.`));
            throw new Error("Command failed");
        }
        if (lock.redeemScriptHash !== deployArtifact.redeemScriptHash) {
            getOutput().error(pc.red(`SILVERSCRIPT_REDEEM_HASH_MISMATCH: Computed redeem hash does not match deploy artifact.`));
            throw new Error("Command failed");
        }
        ...
        spendPlan.contentHash = contentHash;
        spendPlan.artifactId = `silverspendplan-${contentHash.substring(0, 16)}`;
        const outPath = path.resolve(process.cwd(), `${spendPlan.artifactId}.json`);
        await writeArtifact(outPath, spendPlan);
...
export function getSilverSpendCommand() {
    return new Command("spend")
        ...
        const kaspa = await import("kaspa-wasm");
        const createOutputScriptPublicKey = (address, amountSompi) => {
            ...
            const tx = kaspa.createTransaction(dummyUtxos, [{ address, amount: BigInt(amountSompi) }], new kaspa.Address(address), 2000n, undefined, 1, 1);
        ...
        try {
            utxoResponse = await sdk.rpc.requestRaw("getUtxosByAddressesRequest", {
                addresses: [
                    (new kaspa.Address("simnet", kaspa.AddressVersion.ScriptHash, spendPlan.redeemScriptHash)).toString()
                ]
            });
        }
        catch (err) {
        ...
        const rawTx = {
            ...
            outputs: spendPlan.expectedOutputs.map((o) => {
                return {
        ...
        receipt.artifactId = `silverreceipt-${contentHash.substring(0, 16)}`;
        receipt.lineage.artifactId = contentHash;
        const outPath = path.resolve(process.cwd(), `${receipt.artifactId}.json`);
        await writeArtifact(outPath, receipt);
        await sdk.rpc.close();
...
//# sourceMappingURL=silver-lifecycle.js.map