...
export async function runDoctorChecks(root, opts) {
    if (opts.json)
        UI.setJsonMode(true);
    const report = {
    ...
    if (dirExists) {
        ...
        const checkStream = async (streamName, streamPath, schemaValidator) => {
            try {
        ...
        await checkStream("Events Ledger", path.join(process.cwd(), "events.jsonl"), (p) => p && p.schema === HardkasSchemas.Event);
            ...
            if (stats.isDirectory()) {
                ...
                for (const snap of snapshots) {
                    try {
                        ...
                        if (!manifest || !manifest.snapshotVersion)
                            brokenSnapshots++;
                    }
                    catch {
                        brokenSnapshots++;
    ...
    if (opts.strict && report.summary.failed > 0) {
        ...
        throw new HardkasCliError("DOCTOR_FAILED", "Strict mode: Doctor health checks failed.", { exitCode: HardkasExitCode.RUNTIME_FAILURE });
    }
