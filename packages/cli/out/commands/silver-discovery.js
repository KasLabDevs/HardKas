...
export function getSilverDeployCommand() {
    const deployCmd = new Command("deploy-sweep")
        ...
        if (opts.network !== "simnet") {
            getOutput().error(pc.red(`SILVERSCRIPT_NETWORK_UNSUPPORTED: Only simnet is supported.`));
        ...
        if (!pkValue) {
            getOutput().error(pc.red(`Error: Please provide --private-key <hex> for the discovery deploy command.`));
            throw new Error("Command failed");
        }
        if (fromAccount.kind !== "kaspa" && !pkValue) {
            getOutput().error(pc.red(`Error: Deploy requires a real kaspa-private-key account.`));
            throw new Error("Command failed");
        }
        ...
        getOutput().writeLine(pc.dim(`  - Compiled Script: ${compileArtifact.compiledScriptHash}`));
        ...
        try {
            ...
            if (((err instanceof Error) ? ((err instanceof Error) ? err.message : String(err)) : String(err)).includes("Insufficient funds")) {
                getOutput().writeLine(pc.yellow(`  ⚠️ Insufficient funds detected. Falling back to dummy UTXO for discovery.`));
                plan = {
                    from: fromAccount,
                    to: fromAccount,
                    ...
                    inputs: [
                        {
                            ...
                            scriptPublicKey: {
                                version: 0,
                                scriptPublicKey: "2085ba4021785af60436b86cbbaf8653d26ac3afda2ee600845a827a051bf8e738ac"
        ...
        const txSignable = kaspa.createTransaction(utxos, outputs, changeAddress, priorityFee, undefined, 1, 1);
        ...
        const recordAttempt = (responseError, txId) => {
        ...
        try {
            ...
            if (errMsg.includes("orphan")) {
                getOutput().writeLine(pc.green(`SILVERSCRIPT_STANDARD_LOCK_READY`));
                getOutput().writeLine(pc.dim(`(Node accepted standardness, rejected as orphan due to dummy UTXO)`));
...
export function getSilverSpendCommand() {
    const spendCmd = new Command("unlock-sweep")
        ...
        .option("--network <network>", "Network (must be simnet)", "simnet")
        .action(async (spendPlanArtifactPath, opts) => {
        if (opts.network !== "simnet") {
            getOutput().error(pc.red(`SILVERSCRIPT_NETWORK_UNSUPPORTED: Only simnet is supported.`));
            throw new Error("Command failed");
        }
        ...
        const lastDeploy = dump.deployAttempts
            .filter((a) => a.nodeResponse === "ACCEPTED")
            .pop();
        if (!lastDeploy || !lastDeploy.txid) {
            getOutput().error(pc.red("No successful deploy found in hardkas-vs-node-script-shape.json"));
            throw new Error("Command failed");
        }
...
//# sourceMappingURL=silver-discovery.js.map