...
export function registerQueryCommands(program) {
        ...
        try {
            const action = async () => {
                ...
                if (report.corruptedFiles?.length > 0) {
                    console.log("\n  Corrupted Files:");
                    for (const f of report.corruptedFiles)
                        console.log(`    ${pc.red("✗")} ${f}`);
                }
                if (!report.ok) {
                    const cmd = report.storeIssues?.some((i) => i.code.includes("MIGRATION"))
                        ? "migrate"
                        : "rebuild";
                ...
                if (options.json) {
                    ...
                    if (result.issues && result.issues.length > 0) {
                        ...
                        for (const issue of result.issues.slice(0, 10)) {
                            ...
                            if (issue.path)
                                console.log(`      At: ${issue.path}${issue.lineNumber ? ":" + issue.lineNumber : ""}`);
                        }
    ...
    storeCmd
        ...
        .action(async (options) => {
        if (options.backend)
            process.env.HARDKAS_PROJECTION_BACKEND = options.backend;
        if (options.json)
            UI.setJsonMode(true);
        const { withLock } = await import("@hardkas/core");
        try {
                ...
                if (options.json) {
                    ...
                    if (result.issues && result.issues.length > 0) {
                        ...
                        for (const issue of result.issues.slice(0, 10)) {
                            ...
                            if (issue.path)
                                UI.logHuman(`      At: ${issue.path}${issue.lineNumber ? ":" + issue.lineNumber : ""}`);
                        }
                        if (result.issues.length > 10)
    ...
    artifactsCmd
        ...
        try {
            ...
            if (options.mode)
                filters.push({ field: "mode", op: "eq", value: options.mode });
            if (options.from)
    ...
    queryCmd
        ...
        try {
            ...
            if (options.kind)
                filters.push({ field: "kind", op: "eq", value: options.kind });
            if (options.workflow)
                filters.push({ field: "workflowId", op: "eq", value: options.workflow });
            const params = {};
            if (options.tx)
                params["tx"] = options.tx;
            const request = createQueryRequest({
                domain: "events",
                op: "list",
                filters,
...
function printReplaySummary(result) {
    ...
    if (s.preStateHash)
        console.log(`  Pre-state:  ${s.preStateHash.slice(0, 16)}...`);
    if (s.postStateHash)
        console.log(`  Post-state: ${s.postStateHash.slice(0, 16)}...`);
    console.log("");
}
...
function printDagConflicts(result) {
    ...
    for (const c of result.items) {
        ...
        for (const l of c.loserTxIds)
            console.log(`    └─ LOSER:  ${l.slice(0, 24)}...`);
        console.log("");
    }
    printExplain(result.explain);
    printWhy(result.why);
