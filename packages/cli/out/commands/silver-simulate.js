...
import { createSilverSimulationState, simulateSilverDeploy, simulateSilverSpend, SilverSimulationError } from "@hardkas/simulator";
const STATE_PATH = path.join(".hardkas", "silver-simulator", "state.json");
export function getSilverSimulateCommand() {
    const cmd = new Command("simulate").description("Local SilverScript/Toccata artifact lifecycle simulator");
    cmd
        .command("deploy")
    ...
    cmd
        ...
        .option("--mode <mode>", "Comparison mode: artifact-coherence, runtime-outcome, strict", "artifact-coherence")
        .action((opts) => {
...
function loadState() {
    ...
    if (!fs.existsSync(fullPath))
        return createSilverSimulationState();
...
function handleSilverSimulationError(error) {
...
function compareSilverReceipts(simulated, docker, mode) {
    ...
    for (const field of fields) {
        ...
        if (entry.classification === "MATCH")
            continue;
        const nonConsensusRuntimeIds = mode === "runtime-outcome" &&
...
function compareLineageSemantics(simulated, docker, mode) {
    ...
    if (deployHashEntry)
        notes.push(deployHashEntry);
    const spendPlanHashEntry = compareSpendPlanSource(simulated, docker);
    if (spendPlanHashEntry)
        notes.push(spendPlanHashEntry);
    notes.push({
        field: "lineage.blob",
        reason: mode === "artifact-coherence"
...
function compareDeploySource(simulated, docker) {
    if (!simulated.deploySimulationHash && !docker.deployArtifactHash)
        return undefined;
...
function compareSpendPlanSource(simulated, docker) {
...
//# sourceMappingURL=silver-simulate.js.map