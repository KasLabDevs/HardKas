...
function getCompilerPath(explicitPath) {
    if (explicitPath)
...
function checkCompilerReady(compilerPath) {
    try {
...
export function registerSilverCommand(program) {
    ...
    const discoveryCmd = new Command("discovery").description("Experimental SilverScript discovery tools");
    ...
    silverCmd
        ...
        try {
            ...
            getOutput().writeLine(`  ${pc.green("OK")} Compiler: ${pc.bold("SILVERSCRIPT_COMPILER_READY")}`);
            getOutput().writeLine(`  ${pc.green("✅")} Compiler: ${pc.bold("Available")}`);
        }
        catch (err) {
            getOutput().writeLine(`  ${pc.red("❌")} Compiler: ${pc.red("SILVERSCRIPT_COMPILER_UNAVAILABLE")}`);
            getOutput().writeLine(pc.dim(`     (Could not find 'silverc' binary in ${getCompilerPath()})`));
            getOutput().writeLine(pc.dim(`     Hint: pass --compiler-path, set HARDKAS_SILVERC_PATH, or install to .hardkas/bin/silverc.\n`));
        }
        ...
        try {
            ...
            try {
                ...
                if (dockerPs.includes("rusty-kaspad:v2.0.0")) {
                    getOutput().writeLine(`  ${pc.green("✅")} Network: ${pc.bold("TOCCATA_MAINNET_RELEASE_DETECTED")} (v2.0.0 node running)`);
                    getOutput().writeLine(pc.dim("     Node is v2.0.0 Mainnet Toccata Release."));
                    getOutput().writeLine(`  ${pc.yellow("⚠️")} ${pc.yellow("SILVERSCRIPT_MAINNET_NOT_ENABLED")} — script lifecycle restricted to testnet/simnet`);
                }
                else if (dockerPs.includes("rusty-kaspad")) {
                    getOutput().writeLine(`  ${pc.green("✅")} Network: ${pc.bold("Kaspa node detected")} (via Docker)`);
                }
                else {
                    getOutput().writeLine(`  ${pc.yellow("⚠️")} Network: ${pc.yellow("SILVERSCRIPT_NODE_CAPABILITY_UNKNOWN")}`);
                }
            }
            catch {
                getOutput().writeLine(`  ${pc.yellow("⚠️")} Network: ${pc.yellow("SILVERSCRIPT_NODE_CAPABILITY_UNKNOWN")}`);
                getOutput().writeLine(pc.dim("     (Could not determine Kaspa node network capabilities)\n"));
            }
        }
        if (opts.capabilities) {
            ...
            try {
                ...
                getOutput().writeLine(`  Network:            ${blockdag.networkId || "unknown"}`);
                getOutput().writeLine(`  Script Support:     ${version.includes("2.0") ? "TOCCATA_NODE_READY" : "UNKNOWN"}`);
            }
            catch (e) {
    ...
    silverCmd
        ...
        if (!file.endsWith(".sil") && !file.endsWith(".silver")) {
            getOutput().writeLine(pc.yellow(`Warning: Standard SilverScript extension is .sil. You provided: ${file}`));
        }
        if (opts.network !== DEFAULT_NETWORK && opts.network !== "simnet") {
            getOutput().error(pc.red(`Error: SILVERSCRIPT_NETWORK_UNSUPPORTED. The official compiler currently only supports ${DEFAULT_NETWORK}.`));
        ...
        if (!compilerCheck.ok) {
            if (opts.json) {
                ...
                getOutput().error(pc.dim(`Hint: pass --compiler-path, set HARDKAS_SILVERC_PATH, or install to .hardkas/bin/silverc.`));
            }
            throw new Error("Command failed");
        }
        try {
            ...
            getOutput().error((err.stdout)?.toString() || (err.stderr)?.toString() || ((err instanceof Error) ? ((err instanceof Error) ? err.message : String(err)) : String(err)));
            throw new Error("Command failed");
        }
        ...
        artifact.artifactId = `silver-${contentHash.substring(0, 16)}`;
        const artifactPath = path.resolve(process.cwd(), `${artifact.artifactId}.json`);
    ...
    silverCmd
        ...
        if (artifact.schema !== HardkasSchemas.SilverCompile &&
            artifact.schema !== HardkasSchemas.SilverTest &&
            artifact.schema !== HardkasSchemas.SilverSpendPlan) {
            getOutput().error(pc.red(`Error: Expected a SilverScript schema, got ${artifact.schema}`));
            throw new Error("Command failed");
        }
        ...
        if (artifact.schema === HardkasSchemas.SilverCompile ||
            ...
            getOutput().writeLine(`  Compiled Hash: ${pc.cyan(artifact.compiledScriptHash || "Unknown")}`);
            getOutput().writeLine(`  Compiler:      ${artifact.compilerName} ${artifact.compilerVersion}`);
            getOutput().writeLine(`  Network:       ${artifact.network || artifact.networkId}`);
    ...
    silverCmd
        ...
        try {
            ...
            getOutput().writeLine(pc.green(`✅ SilverScript Artifact Verified: ${artifactPath}`));
        }
        catch (err) {
            getOutput().error(pc.red(`❌ Verification failed for ${artifactPath}`));
    ...
    silverCmd
        ...
        const results = [];
        let status = "PARTIAL_TEST_VECTOR_SUPPORT";
        ...
        const sourceHash = createHash("sha256").update(sourceContent).digest("hex");
        let testVectorsHash = null;
        if (opts.vectors) {
            if (!fs.existsSync(opts.vectors)) {
                getOutput().error(pc.red(`Error: Test vectors file not found: ${opts.vectors}`));
                throw new Error("Command failed");
            }
            try {
                ...
                getOutput().error(pc.red(`SILVERSCRIPT_TEST_VECTOR_INVALID: Malformed vectors.json`));
                throw new Error("Command failed");
            }
        }
        // Run compile
        let compileStdout = "";
        try {
            compileStdout = execFileSync(process.execPath, [process.argv[1], "silver", "compile", file].concat(opts.compilerPath ? ["--compiler-path", opts.compilerPath] : []), { encoding: "utf8", stdio: ["pipe", "pipe", "pipe"], shell: false, timeout: 30000 });
            ...
            if (opts.expectedFail) {
                getOutput().writeLine(pc.yellow(`⚠️ Compilation failed as expected (--expected-fail)`));
                ...
                getOutput().error((err.stderr) || ((err instanceof Error) ? ((err instanceof Error) ? err.message : String(err)) : String(err)));
        ...
        testArtifact.contentHash = contentHash;
        testArtifact.artifactId = `silvertst-${contentHash.substring(0, 16)}`;
        const outPath = opts.out
            ? path.resolve(process.cwd(), opts.out)
            : path.resolve(process.cwd(), `${testArtifact.artifactId}.json`);
        await writeArtifact(outPath, testArtifact);
        getOutput().writeLine(pc.green(`\n✅ SilverScript Test Harness Completed`));
        getOutput().writeLine(`Status: ${pc.bold(status === "PASS" ? pc.green(status) : pc.yellow(status))}`);
    ...
    silverCmd
        ...
        try {
            ...
            if (!match)
                throw new Error("Could not find artifact path");
