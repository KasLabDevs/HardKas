...
export function assertBroadcastNetworkAllowed(input) {
    ...
    if (isMainnetArtifact || isMainnetSelected) {
        throw new Error("Mainnet broadcast is disabled in HardKAS 0.12.0-rc.17.\n\n" +
            "Reason:\n" +
    ...
    if (!isSameNetwork(input.artifactNetworkId, input.selectedNetwork)) {
        throw new Error(`Network mismatch: signed artifact targets '${input.artifactNetworkId}' ` +
...
function isSameNetwork(n1, n2) {
    ...
    if (n1 === n2)
        return true;
    if (isMainnetLike(n1) && isMainnetLike(n2))
        return true;
    // Potential future mapping could go here
    return false;
}
//# sourceMappingURL=broadcast-guard.js.map