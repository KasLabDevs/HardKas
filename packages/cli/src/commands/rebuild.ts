      ...
      try {
        ...
        if (!opts.fromArtifacts) {
          throw new HardkasCliError(
            "USAGE_ERROR",
            "Must specify --from-artifacts to confirm rebuild action.",
        ...
        await withLock(
          ...
          async () => {
            ...
            const { HardkasStore, SqliteQueryBackend } =
            ...
            try {
              ...
              if (
                connectErr?.code === "PROJECTION_BUSY" ||
                ...
                throw new HardkasCliError(
                  ...
                  "Projection database is busy. Another process may be using it. Retry later.",
                  {
                    ...
                    suggestion:
                      "Close other HardKAS processes or wait for them to finish, then retry."
                  }
                );
              }
              throw connectErr;
            ...
            if (opts.json) {
              ...
              if (!result.ok) {
                throw new HardkasCliError(
                  "RUNTIME_FAILURE",
                  "Rebuild failed or encountered corruption.",
                  { exitCode: HardkasExitCode.RUNTIME_FAILURE }
                );
              }
              UI.success(
                `Rebuild complete. Indexed ${result.artifacts.indexed} artifacts.`
              );
            }
          }
        );
