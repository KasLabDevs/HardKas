...
export function registerNodeCommands(program: Command) {
  ...
  nodeCmd
    ...
    .option(
      ...
      "Allow using a floating tag like 'latest' without warning",
      false
    )
                ...
                ports: [
                  {
                    protocol: "grpc",
                    host: "127.0.0.1",
                    port: result.status.ports.rpc,
                    ready: result.status.transports.grpc.ready
                  },
                  {
                    protocol: "borsh",
                    host: "127.0.0.1",
                    port: result.status.ports.borshRpc,
                    ready: result.status.transports.borsh.ready
                  },
                  {
                    protocol: "json",
                    host: "127.0.0.1",
                    port: result.status.ports.jsonRpc,
                    ready: result.status.transports.json.ready
                  }
                ],
