    ...
    program
        ...
        try {
            ...
            await withLock({
                ...
                if (!opts.json)
                    UI.info("Rebuilding projections from canonical artifacts...");
                ...
                try {
                    ...
                    if (connectErr?.code === "PROJECTION_BUSY" ||
                        connectErr?.message === "PROJECTION_BUSY") {
                        throw new HardkasCliError("PROJECTION_REBUILD_BUSY", "Projection database is busy. Another process may be using it. Retry later.", {
                            exitCode: HardkasExitCode.RUNTIME_FAILURE,
                            suggestion: "Close other HardKAS processes or wait for them to finish, then retry."
                        });
                    }
                    throw connectErr;
                }
