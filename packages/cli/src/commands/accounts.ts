...
export function registerAccountsCommands(program: Command) {
  ...
  realAccountsCmd
    ...
    .option(
      "--private-key <hex>",
      "Deprecated. Unsafe: may leak through shell history. Prefer --private-key-stdin or --private-key-env."
    )
    ...
    .option(
      "--password-env <env>",
      "Read keystore password from environment variable (safe)"
    )
    .option(
      "--unsafe-plaintext",
    ...
    .action(async (options: any) => {
      ...
      try {
        await withLock(
          ...
          async () => {
            const { runAccountsKeystoreImport } =
              await import("../runners/accounts-keystore-runners.js");
    ...
    .action(async (name: string, options: any) => {
      ...
      try {
        await withLock(
          ...
          async () => {
            const { runAccountsKeystoreChangePassword } =
              await import("../runners/accounts-keystore-runners.js");
  ...
  realAccountsCmd
    .command("generate")
    .description(
      `Generate new real dev account(s) using Kaspa SDK ${UI.maturity("stable")}`
    )
    .option("--name <name>", "Base name for account(s)")
    .option("--count <number>", "Number of accounts to generate", "1")
    .option(
      "--network <network>",
      "Kaspa network (simnet, testnet-10, mainnet)",
      "simnet"
    )
