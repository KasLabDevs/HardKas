    ...
    tx.command("status <path>")
        ...
        try {
            ...
            if (raw.schema === "hardkas.txPlan") {
                ...
                console.log(`Amount:       ${raw.amountSompi} sompi`);
                if (raw.multisig) {
                    console.log(`Threshold:    ${raw.multisig.signatures.length} of ${raw.multisig.threshold} Required Signers`);
            ...
            await withLock({
                ...
                if (signedPath) {
                    ...
                    if (options.json) {
                        ...
                        const { UI } = await import("../ui.js");
                        const sendResult = result.steps.send;
                        const isSimulated = sendResult?.artifact?.rpcUrl === "simulated://local" || options.network === "simulated";
                        UI.causality(isSimulated
                            ? "Transaction simulated successfully"
                            : "Transaction broadcast successfully", {
