...
export function registerTortureCommands(program: Command) {
  ...
  tortureCmd
    .command("matrix")
    .description(
      `Execute the deterministic chaos-and-mutation torture matrix ${UI.maturity("alpha")}`
    )
  ...
  tortureCmd
    .command("replay")
    .description(
      `Replay and debug a specific failed case from a torture run ${UI.maturity("alpha")}`
    )
