...
async function resolveArtifactPath(id) {
...
export function registerExplainCommand(program) {
    program
        ...
        .description(`Provide a narrative causal explanation of a deterministic artifact, transaction, or replay ${UI.maturity("stable")}`)
        .action(async (id) => {
        try {
            const artifactPath = await resolveArtifactPath(id);
            if (!artifactPath) {
                UI.semanticError("Artifact Not Found", `Could not locate artifact with identifier or path '${id}'`, "causal chain integrity", "cannot explain deterministic causality for missing data", "verify the ID and ensure you are in the correct HardKAS workspace");
                process.exitCode = 1;
                return;
            }
