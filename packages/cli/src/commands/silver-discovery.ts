...
export function getSilverDeployCommand() {
  const deployCmd = new Command("deploy-sweep")
    ...
    .description("EXPERIMENTAL: Create a real UTXO locked by the compiled script on simnet")
    ...
    .option("--wrapper <type>", "Locking script wrapper type: raw | kaspa-p2sh-blake2b | hash160 | compiler", "kaspa-p2sh-blake2b")
    ...
    .action(async (compileArtifactPath, opts) => {
        ...
        if (opts.network !== "simnet") {
            console.error(pc.red(`SILVERSCRIPT_NETWORK_UNSUPPORTED: Only simnet is supported.`));
            process.exit(1);
        }
        ...
        if (!pkValue) {
            console.error(pc.red(`Error: Please provide --private-key <hex> for the discovery deploy command.`));
            process.exit(1);
        }

        if (fromAccount.kind !== "kaspa-private-key" && !pkValue) {
            console.error(pc.red(`Error: Deploy requires a real kaspa-private-key account.`));
            process.exit(1);
        }
        ...
        console.log(pc.dim(`  - Compiled Script: ${compileArtifact.compiledScriptHash}`));
        console.log(pc.dim(`  - Amount: ${amountSompi} Sompi`));
        ...
        try {
            ...
            if (err.message.includes("Insufficient funds")) {
                console.log(pc.yellow(`  ⚠️ Insufficient funds detected. Falling back to dummy UTXO for discovery.`));
                plan = {
                    from: fromAccount,
                    to: fromAccount,
                    ...
                    inputs: [
                        {
                            ...
                            scriptPublicKey: { version: 0, scriptPublicKey: "2085ba4021785af60436b86cbbaf8653d26ac3afda2ee600845a827a051bf8e738ac" }, // Dummy P2PKH for alice
                            blockDaaScore: "0",
                            isCoinbase: false
                        }
                    ]
        ...
        const changeAddress = plan.change?.address ? new kaspa.Address(plan.change.address) : undefined;
        const priorityFee = BigInt(plan.estimatedFeeSompi);

        // Build with kaspa-wasm
        const txSignable = kaspa.createTransaction(utxos, outputs, changeAddress, priorityFee);

        console.log(pc.yellow(`  ⚠️ Applying wrapper: ${opts.wrapper}`));
        ...
        if (opts.wrapper === "compiler") {
            console.error(pc.red(`COMPILER_WRAPPER_UNAVAILABLE`));
            process.exit(1);
            ...
            const hash160 = createHash("ripemd160").update(createHash("sha256").update(Buffer.from(compileArtifact.compiledScriptHex, "hex")).digest()).digest("hex");
            ...
            const pyCmd = `python -c "import hashlib; print(hashlib.blake2b(bytes.fromhex('${scriptHex}'), digest_size=32).hexdigest())"`;
            const hash32 = execSync(pyCmd).toString().trim();
            lockingScriptHex = `aa20${hash32}87`; // OP_BLAKE2B OP_DATA_32 <hash> OP_EQUAL
            scriptHash = hash32;
            spkVersion = 0; // AddressVersion.ScriptHash (testing 0 because 8 is unknown by v2.0.0)
            opcodesUsed = "OP_BLAKE2B, OP_DATA_32, OP_EQUAL";
            hashFunction = "BLAKE2B (32 bytes)";
        } else {
            console.error(pc.red(`Unknown wrapper: ${opts.wrapper}`));
            process.exit(1);
        }
        ...
        try {
            ...
            console.error(pc.red(`SILVERSCRIPT_LOCK_REJECTED_BY_NODE`));
        ...
        const dumpPath = path.resolve(process.cwd(), "artifacts/hardkas-vs-node-script-shape.json");
        const dump = fs.existsSync(dumpPath) ? JSON.parse(fs.readFileSync(dumpPath, "utf8")) : { deployAttempts: [], spendAttempts: [] };
        if (!dump.deployAttempts) dump.deployAttempts = [];
        
        function toHex(arr: Uint8Array): string { return Buffer.from(arr).toString("hex"); }
        ...
        try {
            ...
            console.log(pc.green(`SILVERSCRIPT_STANDARD_LOCK_READY`));
            console.log(`Transaction ID: ${pc.bold(submitResult.transactionId)}`);
            recordAttempt(null, submitResult.transactionId);
        } catch (err: any) {
            const errMsg = err.message || String(err);
            if (errMsg.includes("orphan")) {
                console.log(pc.green(`SILVERSCRIPT_STANDARD_LOCK_READY`));
                console.log(pc.dim(`(Node accepted standardness, rejected as orphan due to dummy UTXO)`));
                recordAttempt(null, "orphan-standardness-pass");
            } else {
                console.error(pc.red(`SILVERSCRIPT_LOCK_REJECTED_BY_NODE`));
                console.error(`Node response: ${errMsg}`);
...
export function getSilverSpendCommand() {
  const spendCmd = new Command("unlock-sweep")
    ...
    .action(async (spendPlanArtifactPath: string, opts: any) => {
        ...
        const { Hardkas } = await import("@hardkas/sdk");
        const sdk = await Hardkas.create({ cwd: process.cwd(), autoBootstrap: true, network: "simnet" });
        ...
        const lastDeploy = dump.deployAttempts.filter((a: any) => a.nodeResponse === "ACCEPTED").pop();
        if (!lastDeploy || !lastDeploy.txid) {
            console.error(pc.red("No successful deploy found in hardkas-vs-node-script-shape.json"));
            process.exit(1);
        }
        ...
        console.log(pc.dim(`  - Deploy TxId: ${utxoTxId}`));
        console.log(pc.dim(`  - Compiled Script: ${compiledScriptHex}`));
        ...
        const rawTxBase = {
            ...
            outputs: [{
                ...
                scriptPublicKey: { version: 0, scriptPublicKey: "2085ba4021785af60436b86cbbaf8653d26ac3afda2ee600845a827a051bf8e738ac" } // Alice P2PKH
            }],
            lockTime: 0,
        ...
        for (const candidate of candidates) {
            ...
            console.log(`signatureScript: ${candidate.script}`);
            ...
            try {
                ...
                console.log(pc.green(`SILVERSCRIPT_UNLOCK_SHAPE_FOUND!`));
                ...
                const errMsg = err.message || String(err);
                console.log(pc.red(`Rejected: ${errMsg}`));
                dump.spendAttempts.push({
                    timestamp: new Date().toISOString(),
                    candidateName: candidate.name,
                    signatureScriptHex: candidate.script,
