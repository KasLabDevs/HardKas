    ...
    .action(async (options) => {
      try {
        ...
        for (const lock of locks) {
          ...
          console.log(
            `  ${pc.bold(lock.name.padEnd(12))} ${pc.dim("PID:")} ${lock.metadata.pid.toString().padEnd(6)} ${pc.dim("State:")} ${status}`
          );
    ...
    .action(async (name) => {
      try {
        ...
        if (name) {
          ...
          console.log(
            `  State:   ${lock.isAlive ? pc.green("HELD (live)") : pc.red("HELD (STALE)")}`
          );
          ...
          for (const lock of locks) {
            console.log(
              `  ${pc.bold(lock.name.padEnd(12))}: ${lock.isAlive ? pc.green("HELD") : pc.red("STALE")}`
            );
          }
      ...
      try {
        ...
        for (const lock of locks) {
          if (!lock.isAlive) {
            staleCount++;
            console.log(
              `  ${pc.red("✗")} Stale lock found: ${pc.bold(lock.name)} (PID: ${lock.metadata.pid})`
            );
            console.log(
        ...
        if (staleCount === 0) {
          if (locks.length === 0) {
            ...
            console.log(
              `  ✓ All ${locks.length} active lock(s) are held by live processes.`
            );
          }
        }
        console.log("");
