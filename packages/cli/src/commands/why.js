    ...
    program
        ...
        try {
            ...
            if (!fs.existsSync(artifactsDir)) {
                throw new Error("No artifacts directory found. Run this from a HardKAS workspace.");
            }
            ...
            if (matches.length > 1) {
                throw new Error(`Multiple artifacts match "${artifactId}":\n  ${matches.join("\n  ")}`);
            }
            ...
            while (currentId) {
                ...
                const artifact = await readArtifact(path.join(artifactsDir, currentFile));
                let role = "Unknown";
                if (artifact.schema?.startsWith("hardkas.txPlan"))
            ...
            for (let i = 0; i < reversed.length; i++) {
                ...
                UI.raw(`  ${indent}${prefix} ${isTarget ? "\x1b[32m\x1b[1m" : "\x1b[37m"}${node.role}\x1b[0m`);
