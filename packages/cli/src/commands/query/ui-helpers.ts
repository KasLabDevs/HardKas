...
export function printArtifactList(result: QueryResult<ArtifactQueryItem>): void {
  ...
  for (const item of result.items) {
    const hash = item.contentHash
      ? pc.dim(item.contentHash.slice(0, 12) + "...")
      : pc.red("no-hash");
    const from = item.from?.address
      ? pc.dim(` from:${item.from.address.slice(0, 10)}...`)
      : "";
    console.log(
      `  ${pc.white(item.schema.padEnd(24))} ${pc.dim(item.networkId.padEnd(10))} ${pc.dim(item.mode.padEnd(12))} ${hash}${from}`
    );
  }
  console.log(
  ...
  console.log(
    `  ${pc.green(result.annotations.executionMs + "ms")} | ${pc.blue((result.annotations.filesScanned ?? 0) + " files scanned")}\n`
  );
...
export function printInspectResult(result: QueryResult<ArtifactInspectResult>): void {
  ...
  if (!item) {
    console.log(pc.red("  No artifact found."));
    return;
  }
  ...
  console.log(
    `  ${pc.dim("Hash:")}       ${pc.magenta(item.item.contentHash || "none")}`
  );
  console.log(
    `  ${pc.dim("Integrity:")}  ${item.integrity.ok ? pc.green("✓ VALID") : pc.red("✗ INVALID")}`
  );
  console.log(`  ${pc.dim("Lineage:")}    ${pc.italic(item.lineageStatus)}`);
  console.log(
    `  ${pc.dim("Staleness:")}  ${pc.yellow(item.staleness.classification)} (${item.staleness.ageHours}h)`
  );
  if (item.economics) {
    console.log(
      `  ${pc.dim("Economics:")}  ${item.economics.ok ? pc.green("✓") : pc.red("✗")} mass=${item.economics.massReported} fee=${item.economics.feeReported}`
    );
  }
...
export function printDiffResult(result: QueryResult<ArtifactDiffResult>): void {
  ...
  console.log(pc.bold(pc.magenta(`\n  ═══ Artifact Diff ═══\n`)));
  console.log(
    `  ${pc.dim("Left:")}   ${pc.white(diff.leftSchema)} ${pc.dim("(" + diff.leftPath + ")")}`
  );
  console.log(
    `  ${pc.dim("Right:")}  ${pc.white(diff.rightSchema)} ${pc.dim("(" + diff.rightPath + ")")}`
  );
  if (diff.identical) {
    ...
    const marker =
      entry.kind === "added"
        ...
        : entry.kind === "removed"
          ...
          : pc.yellow("~");
    console.log(
      `  ${marker} ${pc.bold(entry.field)}: ${pc.dim(entry.left ?? "(absent)")} → ${pc.white(entry.right ?? "(absent)")} ${pc.dim("[" + entry.kind + "]")}`
    );
  }
  console.log("");
}
...
export function printLineageChain(result: QueryResult<LineageChainResult>): void {
  ...
  console.log(
    `  ${pc.dim("Complete:")} ${chain.complete ? pc.green("✓ yes") : pc.red("✗ no (missing ancestors)")}`
  );
  ...
  for (let i = 0; i < chain.nodes.length; i++) {
    ...
    console.log(
      `${prefix} ${pc.white(node.schema.padEnd(20))} ${hash} ${pc.blue(node.networkId)}/${pc.dim(node.mode)}`
    );
  }
  console.log("");
  if (result.explain) printExplainChains(result.explain as unknown as ExplainChain[]);
...
export function printTransitions(result: QueryResult<LineageTransition>): void {
  ...
  for (const t of result.items) {
    ...
    console.log(
      `  ${marker} ${pc.white(t.from.schema.padEnd(20))} → ${pc.white(t.to.schema.padEnd(20))} ${pc.dim("[" + t.rule + "]")}`
    );
  }
  console.log("");
  if (result.explain) printExplainChains(result.explain as unknown as ExplainChain[]);
...
export function printOrphans(result: QueryResult<LineageOrphan>): void {
  ...
  for (const o of result.items) {
    console.log(
      `  ${pc.red("✗")} ${pc.white(o.node.schema)} ${pc.dim("[" + o.node.contentHash.slice(0, 12) + "...]")}`
    );
    console.log(
      `    ${pc.dim("Missing parent:")} ${pc.yellow(o.missingParentId.slice(0, 16) + "...")}`
    );
...
export function printReplayList(result: QueryResult<ReplaySummaryResult>): void {
  ...
  for (const r of result.items) {
    const trace = r.hasTrace
      ? pc.green(`trace:${r.traceEventCount}ev`)
      : pc.dim("no-trace");
    const status = r.status === "accepted" ? pc.green(r.status) : pc.yellow(r.status);
    console.log(
      `  ${pc.white(r.txId.slice(0, 20).padEnd(22))} ${status.padEnd(10)} ${pc.cyan(r.amountSompi.padEnd(12))} ${pc.dim("fee:")}${r.feeSompi} ${trace}`
    );
  }
  console.log("");
}
...
export function printReplaySummary(result: QueryResult<ReplaySummaryResult>): void {
  ...
  console.log(
    `  ${pc.dim("Status:")}     ${s.status === "accepted" ? pc.green(s.status) : pc.yellow(s.status)}`
  );
  ...
  console.log(
    `  ${pc.dim("UTXOs:")}      ${pc.green(s.spentUtxoCount + " spent")}, ${pc.green(s.createdUtxoCount + " created")}`
  );
  console.log(
    `  ${pc.dim("Trace:")}      ${s.hasTrace ? pc.green(`yes (${s.traceEventCount} events)`) : pc.dim("none")}`
  );
  if (s.preStateHash)
    console.log(
      `  ${pc.dim("Pre-state:")}  ${pc.dim(s.preStateHash.slice(0, 16) + "...")}`
    );
  if (s.postStateHash)
    console.log(
      `  ${pc.dim("Post-state:")} ${pc.dim(s.postStateHash.slice(0, 16) + "...")}`
    );
  console.log("");
}
...
export function printDivergences(result: QueryResult<ReplayDivergence>): void {
  ...
  for (const d of result.items) {
    console.log(
      `  ${pc.red("✗")} ${pc.bold("[" + d.kind + "]")} tx:${pc.dim(d.txId.slice(0, 16) + "...")}`
    );
...
export function printDagConflicts(result: QueryResult<DagConflict>): void {
  ...
  console.log(
    pc.bold(pc.magenta(`  ═══ DAG Conflicts: ${pc.white(result.total)} ═══\n`))
  );
  for (const c of result.items) {
    console.log(`  ${pc.red("CONFLICT")}: outpoint ${pc.yellow(c.outpoint)}`);
    console.log(
      `    ${pc.dim("├─")} ${pc.green("WINNER")}: ${pc.white(c.winnerTxId.slice(0, 24) + "...")}`
    );
    for (const l of c.loserTxIds) {
      console.log(
        `    ${pc.dim("└─")} ${pc.red("LOSER")}:  ${pc.dim(l.slice(0, 24) + "...")}`
      );
    }
    console.log("");
  }
...
export function printDagDisplaced(result: QueryResult<DagDisplacement>): void {
  ...
  console.log(
    pc.bold(pc.red(`  ═══ Displaced Transactions: ${pc.white(result.total)} ═══\n`))
  );
  for (const d of result.items) {
    const status = d.currentlyAccepted ? pc.green("re-accepted") : pc.red("displaced");
    console.log(
      `  ${pc.red("✗")} ${pc.white(d.txId.slice(0, 24) + "...")} ${pc.bold("[" + status + "]")}`
    );
...
export function printDagHistory(result: QueryResult<DagTxHistory>): void {
  ...
  for (const e of result.items) {
    ...
    console.log(
      `  ${status.padEnd(10)} ${pc.dim("block:")}${pc.white(e.blockId.slice(0, 12) + "...")} ${pc.dim("daa:")}${e.daaScore} ${sinkPath}`
    );
  }
  console.log("");
  if (result.explain) printExplainChains(result.explain as unknown as ExplainChain[]);
...
export function printSinkPath(result: QueryResult<DagSinkPath>): void {
  ...
  for (let i = 0; i < sp.nodes.length; i++) {
    ...
    console.log(
      `${prefix} ${pc.white(n.blockId.slice(0, 16) + "...")} ${pc.dim("daa:")}${n.daaScore} ${pc.dim("txs:")}${n.acceptedTxCount}${genesis}`
    );
  }
  console.log("");
}
...
export function printDagAnomalies(result: QueryResult<DagAnomaly>): void {
  ...
  for (const a of result.items) {
    console.log(
      `  ${pc.red("✗")} ${pc.bold("[" + a.kind + "]")} ${pc.white(a.description)}\n`
    );
  }
  if (result.explain) printExplainChains(result.explain as unknown as ExplainChain[]);
}
...
export function printRpcHealthTimeline(result: QueryResult<any>): void {
  ...
  for (const e of result.items) {
    ...
    console.log(
      `  ${pc.dim("[" + e.ts + "]")} ${pc.white(e.endpoint)} — ${state} ${pc.dim("(score: ")}${pc.bold(e.score)}${pc.dim(")")} ${pc.cyan(e.latencyMs + "ms")}`
    );
...
export function printRpcDegradations(result: QueryResult<any>): void {
  ...
  for (const d of result.items) {
    console.log(
      `  ${pc.red("✗")} ${pc.white(d.startTs)} to ${pc.white(d.endTs)} ${pc.dim("(" + d.durationMs + "ms)")}`
    );
    console.log(`    ${pc.dim("Endpoint:")}    ${pc.white(d.endpoint)}`);
    console.log(
      `    ${pc.dim("Worst State:")} ${pc.red(d.worstState)} ${pc.dim("(lowest score: ")}${pc.bold(d.lowestScore)}${pc.dim(")")}`
    );
...
export function printRpcCorrelation(result: QueryResult<any>): void {
  ...
  const assessColor =
    c.assessment === "optimal"
      ? pc.green
      : c.assessment === "degraded"
        ? pc.yellow
        : pc.red;
  console.log(`  ${pc.dim("Assessment:")}   ${assessColor(c.assessment.toUpperCase())}`);
  console.log(
    `  ${pc.dim("State:")}        ${pc.white(c.stateAtSubmission)} ${pc.dim("(score: ")}${pc.bold(c.scoreAtSubmission)}${pc.dim(", ")}${pc.cyan(c.latencyAtSubmission + "ms")}${pc.dim(")")}`
  );
...
export function printCorrelationBundle(result: QueryResult<any>): void {
  ...
  if (b.rpc) {
    const status =
      b.rpc.assessment === "optimal"
        ? pc.green(b.rpc.assessment)
        : pc.yellow(b.rpc.assessment);
...
export function printExplainChains(chains: ExplainChain[]): void {
  ...
  for (const chain of chains) {
    ...
    for (const step of chain.steps) {
      ...
      if (step.rule)
        console.log(`       ${pc.dim("Rule:")} ${pc.italic(pc.cyan(step.rule))}`);
    }
    console.log(`  ${pc.magenta("→")} ${pc.bold(pc.green(chain.conclusion))}`);
    console.log(
      `  ${pc.dim("[model:")} ${pc.blue(chain.model)}${pc.dim(", confidence:")} ${pc.yellow(chain.confidence)}${pc.dim("]")}\n`
    );
  }
}
