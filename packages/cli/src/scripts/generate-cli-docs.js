...
async function main() {
    ...
    if (isCheck) {
        ...
        try {
            ...
            if (normalize(existingMd) !== normalize(markdown) ||
...
function countSubcommands(cmd) {
    return (cmd.subcommands.length +
        cmd.subcommands.reduce((acc, c) => acc + countSubcommands(c), 0));
}
main().catch((err) => {
