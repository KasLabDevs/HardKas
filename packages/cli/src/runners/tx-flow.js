...
import { runTxSend } from "./tx-send-runner.js";
import { calculateContentHash, HARDKAS_VERSION } from "@hardkas/artifacts";
import { coreEvents, createEventEnvelope, asEventSequence, asArtifactId, asWorkflowId, asCorrelationId, asNetworkId, asTxId } from "@hardkas/core";
import path from "path";
import fs from "fs";
/**
 * Orchestrates the full transaction workflow: plan -> sign -> send.
 */
export async function runTxFlow(input) {
    const { from, to, amount, network, config, url, feeRate, planOnly, sign, send, yes, outDir, name, allowMainnetSigning, workspaceRoot } = input;
    ...
    try {
        ...
        if (workspaceRoot)
            planInput.workspaceRoot = workspaceRoot;
        ...
        if (actualOutDir) {
            const planPath = await saveArtifact(sdk, planArtifact, actualOutDir, name, "plan", from, to, amount);
        ...
        if (shouldSign) {
            ...
            if (actualOutDir) {
                const signedPath = await saveArtifact(sdk, signedArtifact, actualOutDir, name, "signed", from, to, amount);
            ...
            if (shouldSend) {
                if (!yes) {
                    ...
                    if (actualOutDir && sendResult.receipt) {
                        const receiptPath = await saveArtifact(sdk, sendResult.receipt, actualOutDir, name, "receipt", from, to, amount);
...
async function saveArtifact(sdk, artifact, outDir, baseName, suffix, from, to, amount) {
...
//# sourceMappingURL=tx-flow.js.map