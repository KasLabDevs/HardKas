...
export async function runMetamaskNetwork(options) {
    try {
        ...
        const profile = await getL2NetworkProfile({
            ...
            cliOverrides: {
                rpcUrl: networkId === "simnet" || networkId === "localnet"
                    ? "http://127.0.0.1:8545"
                    : undefined,
...
export async function runMetamaskSnippet(options) {
    try {
        const config = await loadHardkasConfig();
        const networkId = config.config.networkId ||
            config.config.defaultNetwork ||
...
export async function runMetamaskAccount(name, options) {
    try {
        const config = await loadHardkasConfig();
...
//# sourceMappingURL=metamask-runner.js.map