...
export async function runL2Balance(
  address: string,
  options: L2AccountOptions
): Promise<void> {
  ...
  const balanceFormatted = formatWeiAsEtherLike(
    balanceWei,
    profile.gasToken,
    profile.nativeTokenDecimals
  );
...
export async function runL2Nonce(
  address: string,
  options: L2AccountOptions
): Promise<void> {
  const { client, profile } = await getClient(options);
