...
export async function runSessionList() {
  try {
    ...
    for (const s of sessions) {
      ...
      console.log(
        `${active} ${pc.white(s.name.padEnd(12))} ${pc.dim(`[L1: ${s.l1.wallet} | L2: ${s.l2.account}]`)}`
      );
    }
    console.log("");
  } catch (e) {
...
export async function runSessionStatus() {
  try {
    ...
    console.log(
      `${l1Status} L1 Wallet:  ${pc.cyan(session.l1.wallet)} (${session.l1.address})`
    );

    const l2Status = l2Found ? pc.green("✓") : pc.red("✗ MISSING");
    console.log(
      `${l2Status} L2 Account: ${pc.cyan(session.l2.account)} (${session.l2.address})`
    );
    ...
    if (!l1Found || !l2Found) {
      ...
      console.log(
        `${pc.dim("Please update your hardkas.config.ts or recreate the session.")}`
      );
    }
    console.log("");
  } catch (e) {
