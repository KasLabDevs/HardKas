...
export async function runLocalWizard(options) {
    try {
        ...
        const networkId = (typeof configRecord["networkId"] === "string"
        ...
        if (!options.json)
            console.log(`${pc.cyan(pc.bold("1. Checking Environment"))}`);
        let profile;
        try {
        ...
        if (!targetAccount) {
            if (options.nonInteractive) {
                ...
                if (options.json)
                    ...
                    console.log(`  ${pc.red("✗")} Account missing (non-interactive mode).`);
                return;
            }
            if (!options.json)
                console.log(`  ${pc.yellow("⚠")} No EVM account named "${options.account}" found.`);
            ...
            if (!options.json)
                console.log(`  ${pc.green("✓")} Account "${targetAccount.name}" found (${targetAccount.address})`);
        }
        // 3. Balance & Funding
        result.step = "balance";
        if (!options.json)
        ...
        if (kasBalance === 0) {
            ...
            if (!options.json)
                console.log(`  ${pc.green("✓")} Balance: ${kasBalance} iKAS`);
        }
        // 4. Final Success
...
//# sourceMappingURL=local-wizard-runner.js.map