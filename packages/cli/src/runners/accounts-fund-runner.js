...
import { loadOrCreateLocalnetState, saveLocalnetState, fundAddress } from "@hardkas/localnet";
export async function runAccountsFund(options) {
    ...
    const isSimulated = networkId === "simulated" ||
        networkId === "localnet" ||
    ...
    if (isSimulated) {
        ...
        const amount = options.amountSompi || 1000n * 100000000n; // Default 1000 KAS
    ...
    if (networkId === "simnet" || networkId === "dev") {
        ...
        throw new Error(`Funding for real simnet (Docker) via faucet requires a miner account. \n` +
...
//# sourceMappingURL=accounts-fund-runner.js.map