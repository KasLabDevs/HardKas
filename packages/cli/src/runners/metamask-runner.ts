...
export async function runMetamaskNetwork(options: { profile: string; json: boolean }) {
  try {
    ...
    const networkId =
      ((config.config as Record<string, unknown>).networkId as string | undefined) ||
      config.config.defaultNetwork ||
      "simnet";

    // Security Gate: No metamask commands on mainnet/testnet
    if (networkId === "mainnet" || networkId.startsWith("testnet")) {
      throw new Error(
        `MetaMask onboarding is only allowed on local development networks (simnet, localnet). Current network: ${networkId}`
      );
    }
    ...
    console.log(
      `  Chain ID:  ${pc.white(parseInt(payload.chainId, 16))} (${payload.chainId})`
    );
    console.log(
      `  Currency:  ${pc.white(payload.nativeCurrency.symbol)} (${payload.nativeCurrency.name})`
    );
    ...
    console.log(
      `${pc.dim("Alternatively, run: ")} ${pc.cyan("hardkas metamask snippet")}\n`
    );
  } catch (e) {
    handleError(e);
  }
...
export async function runMetamaskSnippet(options: { profile: string }) {
  try {
    ...
    if (networkId === "mainnet" || networkId.startsWith("testnet")) {
      throw new Error(
        `MetaMask onboarding is only allowed on local development networks (simnet, localnet).`
      );
    }
    ...
    console.log(
      `${pc.yellow("Copy and paste this snippet into your browser console while MetaMask is open:")}\n`
    );
...
export async function runMetamaskAccount(
  name: string,
  options: { showPrivateKey: boolean; json: boolean; includeSecret: boolean }
) {
  try {
    const config = await loadHardkasConfig();
    const { resolveHardkasAccount, prepareEvmAccountExport } =
    ...
    const includeSecretRequested = options.json
      ? options.includeSecret && options.showPrivateKey
    ...
    if (options.showPrivateKey && exportData.privateKey) {
      ...
      console.log(
        `  ${pc.red("This is a LOCAL DEV ONLY account. Never share private keys.")}\n`
      );
      console.log(`  Private Key: ${pc.white(exportData.privateKey)}`);
    } else {
      console.log(`\n  Private Key: ${pc.dim("[HIDDEN]")}`);
      console.log(
        `  ${pc.dim("Run with ")} ${pc.cyan("--show-private-key")} ${pc.dim("to reveal.")}`
      );
    }
