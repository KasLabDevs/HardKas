...
export async function runTxBatch(options) {
    ...
    try {
        ...
        if (e instanceof Error && e.message.startsWith("Invalid batch file format"))
            throw e;
    ...
    for (let i = 0; i < payments.length; i++) {
        ...
        if (!payment.from || !payment.to || !payment.amount) {
            if (!json)
                console.error(`[${i + 1}/${payments.length}] Skipping invalid payment object (missing from/to/amount).`);
            results.push({
                index: i,
                ok: false,
        ...
        try {
            ...
            if (flowResult.ok) {
                ...
                failCount++;
                if (!json)
                    console.log(`  ✗ Failed: Flow did not complete ok. (plan: ${flowResult.steps.plan.error}, sign: ${flowResult.steps.sign.error}, send: ${flowResult.steps.send.error})`);
            }
        }
        catch (e) {
            failCount++;
