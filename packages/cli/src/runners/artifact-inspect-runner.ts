...
export async function runArtifactInspect(options: ArtifactInspectOptions) {
    ...
    for (const f of allFiles) {
      if (path.basename(f).includes(options.idOrPath)) {
        ...
        try {
          ...
          if (json.txId === options.idOrPath || json.id === options.idOrPath || json.workflowId === options.idOrPath || json.planId === options.idOrPath || json.signedId === options.idOrPath || json.contentHash?.includes(options.idOrPath)) {
            matches.push(f);
          }
        } catch(e) {}
      }
  ...
  const id = artifact.txId || artifact.workflowId || artifact.id || artifact.planId || artifact.signedId;
  const parents = artifact.parents || artifact.parentTxIds || (artifact.parentTxId ? [artifact.parentTxId] : undefined);
  const lineageId = artifact.lineageId;
  const receiptRef = artifact.receiptId || artifact.receiptPath || (type.includes("txPlan") ? `${artifact.txId}.receipt.json` : undefined);
  
  const isCoreArtifact = ["hardkas.txPlan.v1", "hardkas.signedTx.v1", "hardkas.txReceipt.v1", "hardkas.snapshot"].includes(type);
