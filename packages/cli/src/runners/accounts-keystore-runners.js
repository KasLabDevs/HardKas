...
export async function runAccountsKeystoreImport(options) {
    ...
    if (name === "hardkas-local-docker-test-only") {
        ...
        if (isMainnet) {
            throw new Error("Fixture 'hardkas-local-docker-test-only' is strictly blocked on mainnet. (TEST_ONLY_DO_NOT_USE_FOR_FUNDS)");
        }
    }
    // Acquire Private Key
    ...
    if (!options.unsafePlaintext) {
        ...
        const keystore = await KeystoreManager.createEncryptedKeystore({
            address,
            privateKey: finalKey,
            network: address.startsWith("kaspa:") && !address.startsWith("kaspa:sim_")
                ? "mainnet"
                : "devnet"
        }, password, {
            label: name,
...
export async function runAccountsKeystoreChangePassword(options) {
    ...
    const updatedKeystore = await KeystoreManager.changeKeystorePassword(keystore, oldPassword, newPassword);
...
//# sourceMappingURL=accounts-keystore-runners.js.map