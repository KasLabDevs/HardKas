...
export async function runDevDoctor(options) {
    ...
    try {
        ...
        if (config) {
            ...
            if (fs.existsSync(artifactDir)) {
                ...
                try {
                    ...
                    checks.push({
                        ...
                        suggestion: "Run 'hardkas repair --tail' to truncate the corrupted events.jsonl suffix."
                    });
                    finalStatus = "failed";
                }
                ...
                try {
                            ...
                            if (idToHash.has(id)) {
                                ...
                                if (existing.hash === hash) {
                                    checks.push({
                                        ...
                                        suggestion: "Remove the duplicate artifact file or quarantine it, then rebuild projections.",
                                        details: {
                                            artifactId: id,
                                    ...
                                    checks.push({
                                        ...
                                        suggestion: "Remove the conflicting artifact file or quarantine it, then rebuild projections.",
                                        details: {
                                            artifactId: id,
                                            paths: [existing.path, fullPath],
                    ...
                    checks.push({
                        ...
                        suggestion: "Run 'hardkas verify --strict' to identify and quarantine corrupted artifacts."
                    });
                    finalStatus = "failed";
                }
            }
                ...
                checks.push({
                    ...
                    suggestion: "Run a transaction or 'hardkas dev server' to generate the artifact folder."
                });
            ...
            const networkId = typeof config.config.networkId === "string"
            ...
            if (options.release) {
                    ...
                    if (grepTrustless.includes("docs") ||
                        ...
                        if (!grepTrustless.includes("ZK") &&
                            ...
                            checks.push({
                                ...
                                code: "FAKE_L2_CLAIMS",
                                suggestion: "Remove unconditional 'trustless exit' language. ZK bridge is required."
                            });
                            finalStatus = "failed";
                        }
                    }
                ...
                try {
                    ...
                    const grepRawAppend = execSync("git grep 'appendFileSync' packages/core/src || true", // hardkas-append-allow
                    ...
                    if (grepRawAppend && !grepRawAppend.includes("append-coordinator.ts")) {
                        checks.push({
                            ...
                            suggestion: "Replace raw fs.append" + "FileSync with AppendCoordinator.appendAtomic."
                        });
                        finalStatus = "failed";
                    }
                    else {
                ...
                try {
                    ...
                    if (grepNodeFs) {
                        checks.push({
                            ...
                            suggestion: "Configure your bundler to exclude node: imports or use @hardkas/react."
                        });
                        finalStatus = "failed";
                    }
                    else {
                    ...
                    checks.push({
                        ...
                        suggestion: "Ensure your React/Vite config does not polyfill Node core modules."
                    });
                }
                // Read-only sessions
                checks.push({
...
//# sourceMappingURL=dev-doctor-runner.js.map