...
export async function runDevTxSend(options: { from?: string; to?: string; amount?: string; workspaceRoot?: string; quiet?: boolean }) {
  ...
  const planId = (result.steps.plan as any)?.artifact?.planId || (result.steps.plan as any)?.artifact?.artifactId || (result.steps.plan as any)?.artifact?.txId || "unknown";
  const signId = (result.steps.sign as any)?.artifact?.signedId || (result.steps.sign as any)?.artifact?.artifactId || (result.steps.sign as any)?.artifact?.txId || "unknown";
