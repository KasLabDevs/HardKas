...
const IGRA_BRIDGE_ASSUMPTIONS: L2BridgeAssumptions = {
  ...
  trustlessExit: false,
  custodyModel:
    "Phase-dependent bridge custody; verify current bridge implementation before use.",
  ...
  notes: [
    ...
    "HardKAS does not perform bridge operations in 0.8.18-alpha."
  ],
  updatedAt: "2026-05-07T00:00:00Z" // Reference date for Phase 33
};

const BRIDGE_ASSUMPTIONS_REGISTRY: L2BridgeAssumptions[] = [IGRA_BRIDGE_ASSUMPTIONS];
...
export function validateL2BridgeAssumptions(input: unknown): {
  ok: boolean;
  errors: string[];
} {
  const errors: string[] = [];
