# HardKAS 0.8.2-alpha Execution Proof
...
Phase 1 was initiated to verify the P1 and P2 bug fixes. However, the `tx.send` strict validation idempotency test (P1) **failed** due to a runtime bug in the published `@hardkas/sdk@0.8.2-alpha` version from the NPM registry.
...
The execution has been stopped. The release candidate `0.8.2-alpha` cannot be certified at this time.
