...
const releaseNotes = `# HardKAS 0.11.1-alpha Release Notes
...
## [0.11.1-alpha] - ${new Date().toISOString().split('T')[0]}
### Added
...
const rcManifest = `# HARDKAS 0.11.1 ALPHA RC

**Version:** 0.11.1-alpha
**Status:** RELEASE_CANDIDATE_READY
**Phrase:** HardKAS 0.11.1-alpha is a local-first Kaspa builder framework, not a network release.
...
fs.writeFileSync('RELEASE_NOTES_0.11.1-alpha.md', releaseNotes);
fs.writeFileSync('HARDKAS_0.11.1_ALPHA_RC.md', rcManifest);
...
if (results.overall === 'PASS') {
    console.log('\n✅ HARDKAS_0_11_1_ALPHA_RELEASE_CANDIDATE_READY');
} else {
    console.log('\n❌ RC GATES FAILED');
    process.exit(1);
}
