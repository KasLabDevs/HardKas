...
async function runBenchmark() {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    execSync(
      `${tsx} ${runArgs.join(" ")} tx send --from alice --to ${to} --amount ${amount} --network simulated --yes`,
      { stdio: "ignore" }
    );
  }
  ...
  console.log(
    `Shell Loop Duration: ${shellDuration.toFixed(2)} ms (${(shellDuration / 1000).toFixed(2)} s)\n`
  );

  // 2. dev tx generate
  console.log("2. dev tx generate --count 200...");
  const startDev = performance.now();
  execSync(`${tsx} ${runArgs.join(" ")} dev tx generate --count 200`, {
    stdio: "ignore"
  });
  const endDev = performance.now();
  const devDuration = endDev - startDev;
  console.log(
    `dev tx generate Duration: ${devDuration.toFixed(2)} ms (${(devDuration / 1000).toFixed(2)} s)\n`
  );
